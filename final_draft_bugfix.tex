...
Suppose that Assumption (\ref{reg1}) holds. Subject to initial condition $(u,v)_0=(U_0,V_0)\in \mathcal{X}=\real_+^d\times \mathbb R^d$, and assuming that the associated mollified Cartesian dynamics satisfy the localization condition
\[
\lim_{n\to\infty}\mathbb P\!\left(\sup_{0\le t\le T}\|\vec x^{(n)}_t\|\ge n\right)=0 \quad \text{for all } T>0,
\]
there is a unique solution $(u, v)_t$ to (\ref{uvproc}) such that  $u_t>0$ almost surely and $\mean{\sup_{0\le t\le T} \norm{(u,v)_t}^q}<\infty$, for any $T>0$, $q\ge 2$. Further, $(u,v)_t$ is strong Feller and irreducible on $\mathcal X$.
