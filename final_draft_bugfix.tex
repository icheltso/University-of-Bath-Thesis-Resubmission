...
\]
where $G^n \sim \mathcal{N}(0, I_m)$ are independent standard Gaussian vectors. The Euler--Maruyama scheme provides a time-discretization of the SDE. It approximates the solution both in the strong sense (pathwise) and in the weak sense (in expectation). For a bounded measurable test function $f : \mathbb{R}^d \to \mathbb{R}$, such a recursion defines a Markov chain with transition operator
\begin{equation}
P_{\Delta t} f(x) := \mathbb{E}[f(x^{n+1}) \mid x^n = x].
\end{equation}
