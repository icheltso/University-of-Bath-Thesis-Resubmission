...
\mathcal O(K).
\]
Therefore, for fixed \(\zeta>0\),
\[
\min_{k\leq K}\mathbb E\|\nabla \mathcal L(\lambda_k)\|^2
=
\mathcal O\left(\frac{1}{K}\right).
\]
In the limiting case \(\zeta\to0\), the sum behaves like the harmonic series, since
\[
\lim_{\zeta\to0}\frac{1-K^{-\zeta}}{\zeta}
=
\log K.
\]
Thus one obtains the bound
\[
\min_{k\leq K}\mathbb E\|\nabla \mathcal L(\lambda_k)\|^2
=
...
dependence of the convergence bound itself on the conditioning parameter
...
\eqref{tele_ineq2}. First consider the Neumann-series variance contribution in
$D_{3,k}$. From \eqref{varbndthm2}, this term is of the form
\[
\sum_{k=1}^{K}
\frac{\kappa^2}{B_k(2-\eta\mu)^2}.
\]
...
O\left(\frac{1}{\mu^2}\right),
\]
as $\zeta>0$. Thus this contribution is not the dominant term in the
$\mu$-dependence.

The dominant contribution instead comes from the inner-solver error term
$D_{1,k}$. Using Lemma~C.1.1 and $T_k=O(k^{1+\zeta})$, we have, using the
...
\]
Similarly, the remaining terms in $D_{2,k}$ and $D_{3,k}$ contribute at most
lower-order terms in $\mu$. Substituting these estimates into \eqref{tele_ineq2}
gives
\[
\frac{1}{K}\sum_{k=1}^{K}\mathbb{E}\|\nabla \mathcal L(\lambda_k)\|^2
=
