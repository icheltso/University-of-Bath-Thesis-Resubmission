...
We are interested in finding the optimal value of the hyperparameter, that is, a balance between sparsity of the solution, and its visual accuracy. To formalize this, we solve the bi-level problem
\begin{align} 
...
Under these conditions, the chain converges to the invariant distribution from any starting point. 
...
In Part~II, we develop Hadamard--Langevin dynamics, 
which modify the potential structure through reparameterization 
to accommodate sparse priors.
...
$$
is a martingale. This property is ensured under Novikov’s condition (see \cite[Corollary 5.13 of Novikov Condition]{Karatzas1991-xr}). Define a process $\tilde{W}=\left\{\tilde{W}_t=\left(\tilde{W}_t^{(1)}, \ldots\right.\right.$, $\left.\left.\tilde{W}_t^{(d)}\right), \tilde{\mathcal{F}}_t ; 0 \leq t<\infty\right\}$ by
...
\end{equation}
where $\Pi_{\Delta t} \Psi=\Psi-\int_{\mathcal{X}} \Psi \mathrm{d} \pi_{\Delta t}$, $\varsigma_{\Psi, \mathrm{iid}, \Delta t}^2$ refers to the asymptotic variance of i.i.d. samples and $N_{\mathrm{corr}, \Delta t, \Psi}$ quantifies the number of steps required for the process to decorrelate from its previous state. Furthermore, in the limit of small $\Delta t$, the relation between the decorrelation of discrete and continuous dynamics is
...
Define a new probability measure \(\mathbb Q\) on
\(\mathcal F_T\) by
\[
