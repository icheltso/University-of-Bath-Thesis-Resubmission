...
Sparse optimization plays a critical role in compressed sensing,
inverse problems, and machine learning. While $\ell_1$-type regularizers are widely used to promote sparsity, their nonsmooth nature presents analytical and computational challenges, especially in Bayesian sampling. Standard approaches such as unadjusted Langevin algorithms,
Gibbs sampling, and smoothing-based methods may face challenges
when applied to nonsmooth targets, including slow mixing,
discretization error, or approximation bias
\cite{pereyra2016proximal,durmus2018efficient}.
...
The thesis also examines stochastic bilevel optimization for hyperparameter selection. We analyze methods such as \texttt{stocBio} and propose \texttt{stocHOAG}, a stochastic variant of the HOAG algorithm. A central theme of these chapters is the conditioning of the inner optimization problem.
...
\begin{itemize}
    ...
    where we analyze stochastic algorithms such as \texttt{stocBio} and introduce our own stochastic modification of the HOAG algorithm, \texttt{stocHOAG}. A central theme is the
...
\end{comment}
Sparsity has become a cornerstone of modern applied mathematics.
Whether in compressed sensing, inverse problems, or machine learning,
sparsity-promoting regularizers such as the $\ell_1$-norm are essential
tools for recovering interpretable solutions from limited or noisy data
\cite{anha,bach2012optimization}. Yet, while their optimization properties are well studied, their role in Bayesian sampling remains challenging.
...
We establish a rigorous theoretical foundation for these dynamics. In continuous time, we prove results on ergodicity, moment bounds, and finite-time convergence. For discrete implementations, we propose a semi-implicit scheme in cylindrical coordinates and prove convergence of its invariant measure to that of the continuous dynamics, assuming a timestep-uniform geometric ergodicity condition. Additionally, we propose various splitting method-based discretizations in Cartesian coordinates, that we evaluate empirically. These results are complemented by numerical experiments, ranging from low-dimensional regression to high-dimensional classification and deconvolution tasks, where Hadamard-Langevin dynamics is competitive with standard approaches.

In addition to sampling, the thesis also addresses the related but distinct problem of stochastic bilevel optimization for hyperparameter selection. Many sparse models depend critically on the choice of regularization parameters, which can be optimized through bilevel formulations. We analyze existing methods such as \texttt{stocBio} and propose \texttt{stocHOAG}, a stochastic modification of the HOAG algorithm. A central challenge in this setting is the conditioning of the inner problem, for which we provide remedies based on improved sampling approaches. Although these methods do not rely on Hadamard reparameterizations, we emphasize that such reparameterizations can be naturally combined with stochastic bilevel approaches to handle nonsmooth inner problems, pointing toward a promising direction for future research.
...
\begin{enumerate}
    ...
    \item Contributions to stochastic bilevel optimization, including the proposal of \texttt{stocHOAG} and analysis of conditioning issues.
\end{enumerate}
...
All code is available at my Github repository \href{https://github.com/icheltso/thesis}{github.com/icheltso/thesis}.
...
\begin{itemize}
    ...
    bilevel methods such as \texttt{stocBio} and \texttt{stocHOAG}. By highlighting the role of conditioning, and
...
Let $\phi:\mathbb{R}^d\rightarrow\mathbb{R}\cup\{+\infty\}$ be a proper, lower semicontinuous function. The proximal operator of $\phi$ is defined by
\[
\operatorname{prox}_{\phi}(x)
:=
\arg\min_{z\in\mathbb{R}^d}
\left\{
\phi(z)+\frac12\|z-x\|_2^2
\right\}.
\]
\end{definition*}
...
$A$, the notation $A \succ 0$ means that $A$ is positive definite,
i.e. $x^\top A x > 0$ for all nonzero vectors $x$.
\end{definition*}

\begin{definition*}[Posterior distribution]
Let $\theta$ denote an unknown parameter and let $y$ denote the observed data. The posterior distribution is the conditional distribution of $\theta$ given $y$, and is defined by Bayes' theorem as
\[
\pi(\theta\mid y)
=
\frac{\pi(y\mid\theta)\pi(\theta)}
{\pi(y)},
\]
where $\pi(\theta)$ is the prior distribution and $\pi(y\mid\theta)$ is the likelihood.
\end{definition*}

\begin{definition*}[Maximum a posteriori (MAP) estimator]
Given observed data $y$, the maximum a posteriori (MAP) estimator is defined by
\[
\hat{\theta}_{\mathrm{MAP}}
=
\arg\max_{\theta}\pi(\theta\mid y),
\]
that is, the parameter value maximizing the posterior distribution.
\end{definition*}

\begin{definition*}[Stationary distribution]
A probability distribution $\pi$ is called a stationary distribution of a stochastic process (or Markov chain) if the distribution is invariant under the dynamics. That is, if $X_0\sim\pi$, then
\[
X_t\sim\pi
\]
for every $t\ge0$.
...
\begin{figure}[h!]
    ...
    \caption{Aliasing caused by undersampling.\protect\footnotemark}
    \label{fig:aliased}
\end{figure}
\footnotetext{Image adapted from National Instruments:
\url{https://www.ni.com/docs/en-US/bundle/labview/page/lvailnsconcepts/aliasing.html}}
...
\end{align*}
which is exactly the update for proximal gradient descent. For various regularizers $g$, $\mathrm{prox}_{\gamma g}$ can be computed explicitly. For example, for $g = \|x\|_1$, we have $$\mathrm{prox}_{\gamma g}(x) = \mathrm{sign}(x) \max(|x|-\gamma,0).$$ This is illustrated in Figure \ref{fig:prox_lasso}.
...
\begin{itemize}
    \item $R(\beta) = \phi(\beta \circ \beta)$, where $\phi$ is proper, concave and upper semi-continuous, with domain $\mathbb{R}^d_+.$
...
\hfil
    ...
    \end{subfigure}
    \caption{Example of an inverse problem: Reference image (left), noisy observation (center), sparse reconstruction in wavelet (Haar) basis (right). Refer to Figure \ref{fig:deblurry_tiger} for more examples of recovery.}\label{fig:blurry_tiger}
\end{figure}
...
\end{equation*}
Before we solve this problem, we need to consider the hyperparameter $\lambda > 0$, the weight of the regularization term. Note that as $\lambda \xrightarrow[]{} 0$, $x^*$ approaches the solution of the non-regularized least squares problem, while as $\lambda \xrightarrow[]{} \infty$, the regularizer starts to dominate the solution. This trade-off is illustrated in Figure~\ref{fig:deblurry_tiger}. For small values of $\lambda$, the reconstruction is dominated by the data fidelity term and may overfit noise, whereas for large $\lambda$, the regularization term dominates, leading to overly sparse or oversmoothed 
solutions. The right panel shows the corresponding regularization path, highlighting the dependence of the reconstruction quality (measured by SNR) on $\lambda$.

We are interested in finding the optimal value of the hyperparameter, that is, a balance between sparsity of the solution, and its visual accuracy. To formalize this, we solve the bilevel problem
\begin{align} 
...
\end{align}
where $\mathcal{D}$ is non-empty, convex and compact and $C(\cdot,\lambda)$ is some smooth loss function. Here, $L : \mathbb{R}^n \to \mathbb{R}$ denotes a loss function measuring data fidelity, typically of the form $L(Ax, b)$, for example the squared loss $L(Ax,b)=\|Ax-b\|_2^2$. In denoising problems, SNR, or {\em signal-to-noise ratio} is often used \cite{gonzalez2008digital}:
\begin{equation}
...
\begin{figure}[htp]
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        \includegraphics[width=\linewidth]{IMG/tiger_big/tiger_l1_opt.png}
        %\caption{Contact angle with various pseudo dosages}
        \label{fig:tiger_l1_opt}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.4\textwidth}
        \includegraphics[width=\linewidth]{IMG/tiger_big/reg_path_l1.png}
        %\caption{Contact angle with various pseudo dosages}
    ...
    % Second row
    \begin{subfigure}[b]{0.4\textwidth}
        \includegraphics[width=\linewidth]{IMG/tiger_big/tiger_l2_opt.png}
        %\caption{Contact angle with various pseudo dosages}
        \label{fig:tiger_l2_opt}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.4\textwidth}
        \includegraphics[width=\linewidth]{IMG/tiger_big/reg_path_l2.png}
        %\caption{Contact angle with various pseudo dosages}
    ...
    % Third row
    \begin{subfigure}[b]{0.4\textwidth}
        \includegraphics[width=\linewidth]{IMG/tiger_big/tiger_sob_opt.png}
        %\caption{Contact angle with various pseudo dosages}
        \label{fig:tiger_sob_opt}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.4\textwidth}
        \includegraphics[width=\linewidth]{IMG/tiger_big/reg_path_sob.png}
        %\caption{Contact angle with various pseudo dosages}
    ...
    % Fourth row
    %\begin{subfigure}[b]{0.4\textwidth}
    %    \includegraphics[width=\linewidth]{IMG/tiger_big/tiger_tv_opt.png}
    %    %\caption{Contact angle with various pseudo dosages}
    %    \label{fig:tiger_tv_opt}
    %\end{subfigure}
    %\hfill
    %\begin{subfigure}[b]{0.4\textwidth}
    %    \includegraphics[width=\linewidth]{IMG/tiger_big/reg_path_tv.png}
        %\caption{Contact angle with various pseudo dosages}
    %    \label{fig:tiger_tv_path}
    %\end{subfigure}

    \caption{Image deconvolution by regularization; Reference original and blurred images found in Figure \ref{fig:blurry_tiger}. Left: Reconstructions corresponding to the SNR-optimal value of $\lambda$;  Right: Regularization paths showing SNR as a function of $\lambda$.}\label{fig:deblurry_tiger}
\end{figure}

\section{Hyperparameter Optimization with Smooth Inner Problem}
\label{section-hoag}
Recall the bilevel problem
\begin{align*} 
...
\end{align*}
We consider the case where both $C$ and $F$ are smooth. 
...
In the next section, we investigate the HOAG algorithm by Pedregosa, wherein we conduct gradient descent using an approximate hypergradient. Recall that $F : \mathbb{R}^n \times \mathcal{D} \to \mathbb{R}$ is defined in \eqref{inprob} as the inner objective, and $C : \mathbb{R}^n \times \mathcal{D} \to \mathbb{R}$ is the outer objective defined in \eqref{bilev1}.
...
\begin{itemize}
    \item The functions $C$ and $F$ are twice continuously differentiable. 
    Moreover, the gradients $\nabla C(x,\lambda)$ and the Hessian 
    $\nabla_{xx}^2 F(x,\lambda)$ are globally Lipschitz continuous 
    with respect to $(x,\lambda)$ on 
    $\mathbb{R}^n \times \mathcal{D}$.

    \item The hyperparameter domain $\mathcal{D}$ is a non-empty, convex, 
    and compact subset of $\mathbb{R}^r$.

    \item For each $\lambda \in \mathcal{D}$, the Hessian 
    $\nabla_{xx}^2 F(x(\lambda),\lambda)$ is invertible.
\end{itemize}
...
\end{equation}
Denote $\nabla x(\lambda) = \frac{dx(\lambda)}{d\lambda}$. Differentiating the first-order optimality condition with respect to $\lambda$, and using the chain rule, we obtain
...
Here, the error sequence $(\varepsilon_k)_{k \ge 0}$ must be summable, otherwise we might have divergence in the outer problem. Hence we have the complexity-accuracy trade-off between solving the inner problem and solving the outer problem. To be more precise, we present Pedregosa's main results about the convergence of HOAG.
...
In the HOAG algorithm, the computation of the hypergradient requires
solving the linear system
\[
\nabla^2_{xx} F(x_k,\lambda_k) q_k
= \nabla_x C(x_k,\lambda_k),
\]
which involves second-order information of the inner objective. Rather than explicitly forming or inverting the Hessian, this system is typically solved using an iterative method such as conjugate gradients. Although CG avoids forming the full Hessian, each iteration requires Hessian--vector products. In many inverse problems, such products have cost comparable to evaluating a gradient, often $\mathcal{O}(mn)$ when $A \in \mathbb{R}^{m \times n}$. Consequently, the overall cost of solving the linear system depends on the number of CG iterations, which in turn depends on the conditioning of $\nabla^2_{xx} F$. When the Hessian is ill-conditioned, the number of required iterations may become large, increasing the computational burden of each hyperparameter update. This motivates the use of alternative strategies that approximate second-order information more efficiently. One such approach is to employ quasi-Newton approximations, such as the symmetric rank-one (SR1) update, which construct an approximation of the Hessian (or its inverse) using only first-order information.
...
\begin{equation} \label{neumann_orig} 
    A^{-1} = \sum^{\infty}_{i=0} (I-A)^i.
\end{equation}
There are two consequences to this result. Firstly, we can use (\ref{neumann_orig}) to approximately solve the linear system in Algorithm \ref{alg:hoag} by matrix-vector multiplications. Inverting an $m \times m$ Hessian would have a complexity of $\mathcal{O}(m^3)$, while leveraging an $L-$term Neumann series would have $\mathcal{O}(Lm^2)$, where we generally have $L<m$, especially for large datasets. However a more important consequence, is that we can derive a stochastic modification of the HOAG method. Recall the formulated problem 
\begin{align} 
&\min _{\lambda \in \mathcal{D} \subset \mathbb{R}^r} L\left(\lambda \right) \triangleq C(\hat{x}(\lambda), \lambda) \label{outer_hoag_neu_orig}\\
&\text { s.t. } \hat{x}(\lambda) =\underset{x \in \mathbb{R}^{n}} \argmin {F(x, \lambda)}. \label{inner_hoag_neu_orig}
\end{align}
...
Ji, Yang \& Liang have further developed two algorithms - MRBO and VRBO, building upon \texttt{stocBio}\cite{yang2021provably}. The former is a single loop algorithm, that uses momentum-based techniques to keep a running average of gradient estimations at each step, which are used to simultaneously update $x_k$ and $\lambda_k$. This method uses a decreasing stepsize for the updates. The latter is a double-loop algorithm that uses variance reduction techniques, similar to SVRG (Johnson \& Zhang \cite{johnson2013accelerating}). The outer loop is divided into epochs, while the inner loop, much like in the case of \texttt{stocBio}, solves the inner problem (\ref{inner_hoag_neu_orig}), using the relevant gradients that are updated once per epoch with a relatively large batch of samples $\mathcal{S}_1$. Between the epochs the gradients are continuously updated using small batches of samples $\mathcal{S}_2$. To elaborate, the hypergradient $\nabla \tilde{L}$, when evaluated at the start of a new epoch, takes the form
...
&\text { s.t. } \hat{x}(\lambda) =\underset{x \in \mathbb{R}^{n}} \argmin {F(x,\lambda) \triangleq f(x) + \sum_{j=1}^\infty g_j(x,\lambda)} \label{inprob_ns0},
\end{align}
...
has all positive eigenvalues $|\lambda_i|^2 + 1$, where $\lambda_i$ denotes the eigenvalue of $A\mathrm{diag}(v)$. Hence $\partial_{uu} G(u,v)$ is invertible. By the implicit function theorem, $u$ can be locally expressed as a 
function of $v$. Differentiating with respect to $v$, we obtain
\begin{equation}\label{eq: fvphess}
    \nabla^2 f_{vp}
    = \partial_{vv} G(u,v)
    + \partial_{uv}G(u,v)\,\partial_v u.
\end{equation}
Using the relation
\[
\partial_v u
= [\partial_{uu}G(u,v)]^{-1}\partial_{vu}G(u,v),
\]
we obtain
\[
\nabla^2 f_{vp}
...
\partial_{vu}G(u,v).
\]
Thus, the Hessian $\nabla^2 f_{vp}$ coincides with the Schur complement 
of $\nabla^2 G$, namely
\[
\nabla^2 G =
\begin{pmatrix} 
\partial_{vv} G(u,v) & \partial_{uv} G(u,v) \\
\partial_{vu} G(u,v) & \partial_{uu} G(u,v)
\end{pmatrix},
\]
...
\end{align}
which we can tackle as in the HOAG framework (Algorithm \ref{alg:hoag}). 
\begin{comment}
    \begin{remark}
...
\begin{remark}
    ...
    \begin{equation} \label{reg_prob_poon_remark}
        \min_{x \in \mathbb{R}^n} \frac{1}{\lambda} L(Ax,b) + R(x),
    \end{equation}
    where the regularizer $R$ has a quadratic variational form as in (\ref{qvf_thm}), using the smooth reparametrization (\ref{noncvx_varpro}) with LBFGS-B often performs better than, or at least comparably to, state-of-the-art method that tackle (\ref{reg_prob_poon_remark}) directly. In particular, this approach is less sensitive to the choice of $\lambda$.
...
\begin{comment}
    \begin{mdframed}[hidealllines=true,backgroundcolor=yellow!20]
    Think of expanding this and add link to \texttt{stocBio}/StocHOAG section.
\end{mdframed}
\end{comment}
...
\end{itemize}
Bayes' theorem provides a framework for statistical inference, enabling us to incorporate prior information into the analysis and systematically update our beliefs as new data becomes available. In many practical applications, the posterior distribution
$p(\theta\mid \mathcal{D})$ cannot be evaluated in closed form because the
marginal likelihood $p(\mathcal{D})$ requires integration over all possible
values of $\theta$:
\begin{equation}
...
Markov Chain Monte Carlo (MCMC) methods are a class of algorithms designed to sample 
from a probability distribution when direct sampling is infeasible. In Bayesian inference, 
the objective is to generate samples from the posterior distribution 
$\pi(\theta) = p(\theta \mid D)$, which is often known only up to a normalization constant.

MCMC constructs a Markov chain $\{\theta^{(k)}\}_{k \ge 0}$ with transition kernel 
$P(\theta' \mid \theta)$ such that the target distribution $\pi$ is invariant. 

Throughout this thesis we consider Markov chains on continuous state spaces, typically $\mathbb{R}^d$. In this setting, probability masses are replaced by probability
densities, and sums over states are replaced by integrals with respect to
the underlying measure.
A probability measure $\pi$ is said to be invariant for the chain if

\begin{equation} \label{eq:invariance_ker}
    \pi(\theta') 
= \int P(\theta' \mid \theta)\,\pi(\theta)\,d\theta.
\end{equation}

If, in addition, the chain is ergodic, then for any initial condition $\theta^{(0)}$,

\[
\theta^{(k)} \xrightarrow[k\to\infty]{\mathcal{D}} \pi.
\]

Ergodicity typically requires:
\begin{itemize}
    \item \textbf{Irreducibility:} every region of the state space can be reached with positive probability;
    \item \textbf{Aperiodicity:} the chain does not evolve in deterministic cycles.
\end{itemize}

Under these conditions, the chain converges to the invariant distribution from any starting point\cite{robert2004monte,meyn2012-oq}. 

\subsection{Detailed Balance and Reversibility}

A common way to ensure that a distribution $\pi$ is invariant is to impose the 
\emph{detailed balance condition} \cite{pavliotis_book}:

\begin{equation}
\pi(\theta)\,P(\theta' \mid \theta)
=
\pi(\theta')\,P(\theta \mid \theta').
\label{eq:detailed_balance}
\end{equation}

This condition states that, at equilibrium, the probability flow from $\theta$ to 
$\theta'$ equals the reverse flow. A Markov chain satisfying 
\eqref{eq:detailed_balance} is said to be \emph{reversible} with respect to $\pi$.

The detailed balance condition has several important consequences:

\begin{enumerate}
    \item \textbf{Invariance.} Integrating both sides of 
    \eqref{eq:detailed_balance} over $\theta$ shows that $\pi$ satisfies the invariance equation (\ref{eq:invariance_ker}). 
    Thus detailed balance provides a constructive way to design Markov chains with a prescribed stationary distribution.
    
    \item \textbf{Symmetry of the Markov operator.} 
    The associated Markov operator is self-adjoint in $L^2(\pi)$, which simplifies 
    the analysis of convergence via spectral methods.
    
    \item \textbf{Algorithmic construction.}
    Many classical MCMC algorithms, including Metropolis--Hastings and Gibbs sampling, 
    are designed to satisfy detailed balance. When detailed balance, irreducibility, 
    and aperiodicity hold, ergodic convergence to $\pi$ follows.
\end{enumerate}

Although detailed balance is not strictly necessary for invariance, it is widely used 
because of its mathematical simplicity and robustness.

\subsection{The Metropolis--Hastings Algorithm}

A general strategy for constructing a Markov chain with invariant distribution $\pi$ 
is the Metropolis--Hastings algorithm \cite{hastings1970}.

Let $q(\theta' \mid \theta)$ be a proposal density. 
Given the current state $\theta^{(k)}$, a candidate $\theta'$ is generated from $q(\theta' \mid \theta^{(k)})$. 
...
If the proposal is accepted, we set $\theta^{(k+1)} = \theta'$. 
Otherwise, we retain the current state: $\theta^{(k+1)} = \theta^{(k)}$.

The resulting transition kernel satisfies the detailed balance condition
with respect to $\pi$, and therefore $\pi$ is invariant for the Markov chain.
...
\]
The Gibbs sampler generates a Markov chain $\{x^{(t)}\}_{t \geq 0}$ as follows \cite{geman1984stochastic}:
\begin{enumerate}
...
\section{Langevin-Based MCMC}

While classical MCMC methods, such as Gibbs sampling and Metropolis--Hastings, rely on transition kernels constructed to satisfy 
detailed balance, it is often advantageous to exploit gradient information of the target density. 
This leads to Langevin-based methods, which combine stochastic differential equations 
with MCMC corrections.
...
where $U : \mathbb{R}^d \to \mathbb{R}$ is a differentiable potential.
...
Under suitable regularity and growth conditions on $U$, this diffusion is ergodic 
with invariant distribution $\pi$ \cite{pavliotis_book}. Its infinitesimal generator

\[
\mathcal{L} f = -\nabla U \cdot \nabla f + \Delta f
\]

is symmetric in $L^2(\pi)$, meaning that the diffusion is reversible with respect to $\pi$.

Since exact simulation of \eqref{eq:langevin_sde} is generally not possible, 
we discretize it in time.

\subsection{Unadjusted Langevin Algorithm (ULA)}

Applying the Euler--Maruyama scheme with timestep $h > 0$ yields
...
This defines the \emph{Unadjusted Langevin Algorithm} (ULA). 
Each step consists of a gradient descent move combined with Gaussian noise.

ULA is computationally inexpensive, requiring one gradient evaluation per iteration. 
However, it does not exactly preserve $\pi$ at the discrete level. Instead, 
it admits an invariant distribution $\pi_h$ satisfying $\pi_h \to \pi$ as $h \to 0$ \cite{wibisono}. 
Thus ULA introduces a discretization bias.

\subsection{Metropolis-Adjusted Langevin Algorithm (MALA)}

To remove the discretization bias, one can apply a Metropolis--Hastings correction 
to the ULA proposal.

Given the proposal

\[
Y = X_k - h\nabla U(X_k) + \sqrt{2h}\,\xi,
\]
...
where $q(x,y)$ denotes the Gaussian proposal density induced by \eqref{eq:ula}.

This defines the \emph{Metropolis-Adjusted Langevin Algorithm} (MALA). 
By construction, MALA satisfies detailed balance and therefore preserves 
$\pi$ exactly. 

Langevin-based methods are particularly effective in moderate to high dimensions, 
as gradient information guides exploration of the state space. 

Classical Langevin methods are typically developed for smooth
potentials. However, many sparsity-promoting priors lead to
nonsmooth target densities, making the direct application of classical Langevin methods challenging. Part II introduces Hadamard-Langevin dynamics,
which address this issue through a suitable reparameterization of
...
Brownian motion is a $(\mathcal{F}_t)_{t \geq 0}$-martingale, meaning that its expected future value given past observations remains equal to its current value:
\begin{equation}
    \mathbb{E}[W_t \mid \mathcal{F}_s] = W_s, \quad \text{for } s \leq t.
\end{equation}
...
\]
where $(M_t)_{t \geq 0}$ is a positive martingale like the exponential martingale discussed below. This is the key object in Girsanov’s theorem, allowing us to shift the drift of Brownian motion.

\begin{definition}[Exponential Martingale]
Let $(\Omega, \mathcal{F}, (\mathcal{F}_t)_{t \geq 0}, \mathbb{P})$ be a filtered probability space and let $(W_t)_{t \geq 0}$ be a standard Brownian motion adapted to $(\mathcal{F}_t)$. Suppose $X = (X_t)_{t \geq 0}$ is an $(\mathcal{F}_t)$-adapted process such that
\[
...
\medskip
Under appropriate conditions, $M_t$ is a true martingale, and plays the role of a Radon--Nikodym derivative for a change of measure that modifies the drift of $W_t$.
\end{definition}
...
$$
Assume that the process $Z(X)$ defined by 
$$
Z_t(X) \triangleq \exp \left[\sum_{i=1}^d \int_0^t X_s^{(i)} d W_s^{(i)}-\frac{1}{2} \int_0^t\left\|X_s\right\|^2 d s\right]
$$
is a martingale. This property is ensured under Novikov’s condition (see \cite[Corollary 5.13 of Novikov Condition]{Karatzas1991-xr}). 
For each fixed \(T<\infty\), define a probability measure
\(\tilde{\mathbb P}_T\) on \((\Omega,\mathcal F_T)\) by
\[
\frac{d\tilde{\mathbb P}_T}{d\mathbb P}\bigg|_{\mathcal F_T}
=
Z_T(X).
\]
Define a process $\tilde{W}=\left\{\tilde{W}_t=\left(\tilde{W}_t^{(1)}, \ldots\right.\right.$, $\left.\left.\tilde{W}_t^{(d)}\right), \tilde{\mathcal{F}}_t ; 0 \leq t<\infty\right\}$ by
...
For each fixed $T \in[0, \infty)$, the process $\left\{\tilde{W}_t, \tilde{\mathcal{F}}_t ; 0 \leq t \leq T\right\}$ is a d-dimensional Brownian motion on ( $\Omega, \mathcal{F}_T, \tilde{\mathbb{P}}_T$ ).
\end{theorem}
...
Having introduced the probabilistic tools required for changes of measure,
we now review stochastic differential equations and It\^{o} calculus, which
provide the mathematical framework for the continuous-time dynamics studied
...
\begin{equation} \label{sde_eqn_diff}
    dX_t = \mu(X_t, t) dt + \sigma(X_t, t) dW_t, \quad X(0) = x,
\end{equation}
for $X \in \mathbb{R}^d$, $t \in [0,T]$, where $\mu(X_t, t): \mathbb{R}^d \times [0,1] \xrightarrow[]{} \mathbb{R}^d$ and $\sigma(X_t, t): \mathbb{R}^d \times [0,1] \xrightarrow[]{} \mathbb{R}^{d \times n}$ are measurable functions that represent the drift and diffusion terms and $W_t$ is a vector of $n$ independent Brownian motions. (\ref{sde_eqn_diff}) is the differential form of the stochastic integral equation
\begin{equation} \label{sde_eqn_int}
    X_t = x + \int_0^t \mu(X_s, s) ds + \int_0^t\sigma(X_s, s) dW(s).
\end{equation}
It\^{o} calculus provides the framework for analyzing stochastic integrals and differential equations. The It\^{o} integral of a process $f(t)$ with respect to Brownian motion $W_t$ is defined as the limit in $L_2(\Omega)$ of stochastic Riemann sums:
\begin{equation} \label{ito_int}
    \int_0^t f(s) dW_s = \lim_{K \xrightarrow[]{} \infty} \sum_{k=0}^{K-1} f(t_k) (W(t_{k+1}) - W(t_k)),
\end{equation}
where $f$ is an $\{\mathcal{F}_t\}$-adapted process, generated by the Brownian motion $W_t$. Assuming $f$ is square-integrable, the It\^{o} Integral satisfies the It\^{o} isometry \cite{pavliotis_book}:
\begin{equation} \label{ito_isometry}
    \mathbb{E} \left[ \int_0^T f(t) dW(t)\right]^2 = \int_0^T \mathbb{E}\left[|f(t)|\right]^2 dt.
\end{equation}
Furthermore, the It\^{o} Integral is a martingale, so that 
\begin{equation} \label{ito_martin}
\mathbb{E}\left[\int_0^t f(\ell) d W(\ell) \mid \mathcal{F}_s\right]=\int_0^s f(\ell) d W(\ell) \quad \text{ for all } t \geqslant s.
\end{equation}
In stochastic calculus differentiation is redefined via It\^{o}'s lemma:
\begin{equation} \label{ito_lem}
    dF(X_t) = \frac{\partial F}{\partial x} dX_t + \frac{1}{2} \frac{\partial^2 F}{\partial x^2} (dX_t)^2.
\end{equation}
With regards to SDE \eqref{sde_eqn_diff}, we can expand the above to obtain a more explicit form. For twice differentiable $F$,
\begin{equation} \label{ito_lem_exp}
    dF(X_t) = \left\{(\nabla_x F)^T \mu_t(X_t) + \frac{1}{2} [\sigma_t(X_t)\sigma_t(X_t)^T (H_x F) ] \right\} \,dt + (\nabla_x F)^T \sigma_t(X_t) dW_t,
\end{equation}
...
Let $(W_t)_{t\ge0}$ be a one-dimensional Brownian motion on a filtered probability
space $(\Omega,\mathcal F,(\mathcal F_t)_{t\ge0},P)$. The geometric Brownian motion
$(S_t)_{t\ge0}$ is defined as the solution to the stochastic differential equation
\begin{equation}
dS_t = \mu S_t\,dt + \sigma S_t\,dW_t, \quad t \ge 0,
\end{equation}
with initial condition $S_0 = s_0 > 0$, where $\mu \in \mathbb{R}$ and $\sigma > 0$
are constants. This process is widely used to model stock prices in financial
mathematics.

\paragraph{Ornstein--Uhlenbeck Process.}
Let $(W_t)_{t\ge0}$ be a one-dimensional Brownian motion. The Ornstein--Uhlenbeck
process $(X_t)_{t\ge0}$ satisfies the stochastic differential equation
\begin{equation}
dX_t = \theta(\mu - X_t)\,dt + \sigma\,dW_t, \quad t \ge 0,
\end{equation}
with initial condition $X_0 = x_0$, where $\theta > 0$, $\mu \in \mathbb{R}$ and
$\sigma > 0$ are constants. The process models mean-reverting dynamics and is
frequently used in physics and finance.

\section{Generators and Kolmogorov Equations}
Consider the SDE
\begin{equation} \label{aut_sde}
    dX_t = \mu(X_t) dt + \sigma(X_t) dW_t, \quad X(0) = x.
\end{equation}
...
\begin{equation} \label{ito_lem_gen}
    dF(X_t) = \mathcal{L}F dt + (\nabla_x F)^T \sigma(X_t) dW_t,
\end{equation}
with
\begin{equation} \label{generator_explicit}
    \mathcal{L} = \mu(x) \cdot \nabla + \frac{1}{2}\sigma(x)\sigma(x)^T : \nabla^2
\end{equation}
 where $\nabla$ and $\nabla^2$ denote the Jacobian and Hessian operators, respectively, and $:$ denotes the Frobenius inner product. Taking expectations and using the martingale property of the It\^{o} integral (see \eqref{ito_martin}), we obtain
\begin{equation}
\mathbb{E}^x\!\left[F(X_t) - F(X_0)\right] = \mathbb{E}^x\!\left[\int_0^t \mathcal{L}F(X_s)\, ds \right]. \label{generator_martin}
\end{equation} 
Finally, by continuity of $F$, we obtain
...
Let $(W_t)_{t\ge0}$ be a $d$-dimensional Brownian motion.
The overdamped Langevin dynamics is defined by the SDE
\[
dx_t = -\nabla V(x_t)\,dt + \sqrt{2\beta^{-1}}\, dW_t, \qquad t \ge 0,
\]
with initial condition $x_0 \in \mathbb{R}^d$ and $\beta > 0$. The Forward equation is
    $$
...
Let $\mathcal{D} \subseteq \mathbb{R}^d$ denote the state space of the process $x_t$. In the overdamped Langevin setting considered here, $\mathcal{D} = \mathbb{R}^d$. Let $\pi$ denote a probability measure on $\mathcal{D}$ with density proportional to
$e^{-\beta V(x)}$, i.e.
\[
\pi(dx) \propto e^{-\beta V(x)}\,dx .
\]
To study the convergence rate, it is useful to introduce the weighted $L^2$ space:
...
We briefly recall several notions used to measure convergence of probability measures, including relative entropy, Fisher information and total variation distance. For further background see \cite{bakry2014analysis, pavliotis_book}.
...
A probability measure $\pi_2$ satisfies the logarithmic Sobolev (or log-Sobolev) inequality with constant $R > 0$ (denoted LSI($R$)) if, for all probability measures $\pi_1$ such that $\pi_1 \ll \pi_2$, the following inequality holds:
...
Theorem \ref{thm:exp-conv-linf} will be used to demonstrate exponential convergence of Hadamard-Langevin dynamics in Part \ref{part:Langevin} (See Theorem \ref{num_erg}). Having introduced the continuous-time theory, we now turn to the numerical approximation of SDEs. We first review finite-time notions of weak and strong convergence, before discussing long-time properties such as ergodicity, convergence of time averages, and the asymptotic bias of invariant measures. These concepts form the theoretical basis for the analysis of the numerical schemes studied in Part \ref{part:Langevin}.
\section{Convergence of SDE Discretizations}

\subsection{Introduction}

Consider a stochastic process $(x_t)_{t\ge0}$ with values in $\mathbb{R}^d$ defined as the solution to the It\^{o} stochastic differential equation
\[
dx_t = b(x_t)\,dt + \sigma(x_t)\,dW_t,
\]
with initial condition $x_0 \in \mathbb{R}^d$, drift $b: \mathbb{R}^d \to \mathbb{R}^d$, diffusion $\sigma: \mathbb{R}^d \to \mathbb{R}^{d \times m}$ and $W_t \in \mathbb{R}^m$ a standard $m$-dimensional Brownian motion. In practice, such processes are rarely solvable in closed form and must be approximated via numerical discretization schemes. For example, a commonly used time-stepping scheme is the Euler–Maruyama method. Let $t_n = n\Delta t$ with timestep $\Delta t > 0$. The scheme is given by
\[
x^{n+1} = x^n + b(x^n)\Delta t + \sigma(x^n)\sqrt{\Delta t}\, G^n,
\]
where $G^n \sim \mathcal{N}(0, I_m)$ are independent standard Gaussian vectors. The Euler--Maruyama scheme provides a time-discretization of the SDE. It approximates the solution both in the strong sense (pathwise) and in the weak sense (in expectation). For a bounded measurable test function $\Psi : \mathbb{R}^d \to \mathbb{R}$, such a recursion defines a Markov chain with transition operator
\begin{equation}
P_{\Delta t} \Psi(x) := \mathbb{E}[\Psi(x^{n+1}) \mid x^n = x].
\end{equation}
...
\end{array}\right]}_O,
\end{equation}
This way, Leimkuhler and Matthews developed the ABO/BAOAB/ABOBA methods \cite{leimkuhler2024abo,leimkuhler2013rational}. We denote by $\varphi_t^A$, $\varphi_t^B$, and $\varphi_t^O$ the flows
associated with the subsystems $A$, $B$, and $O$, respectively. For example, the BAOAB scheme can be described as
...
%\end{equation}
\begin{equation} \label{eq:baoab_flow}
\varphi_{h/2}^{B} \circ \varphi_{h/2}^{A} \circ \varphi_{h}^{O}
\circ \varphi_{h/2}^{A} \circ \varphi_{h/2}^{B}.
\end{equation}
...
\end{equation}
As shown in \cite{leimkuhler2013rational}, this scheme achieves $\mathcal{O}(h^2)$ accuracy for stationary averages.
...
Weak convergence is concerned with the convergence of expectations of functionals of the solution, which is often sufficient in applications such as statistical estimation or sampling. In contrast, strong convergence focuses on path-wise accuracy, measuring how closely the trajectories of the numerical approximation follow those of the exact solution. The distinction between these two types of convergence plays a critical role in choosing and analyzing discretization methods. Let $(x_t)_{t\ge0}$ denote the solution of the SDE
\[
dx_t = b(x_t)\,dt + \sigma(x_t)\,dW_t,
\]
and let $x^n$ denote the numerical approximation at time $t_n = n\Delta t$ obtained with timestep $\Delta t$.

\subsubsection{Weak Convergence}

Let $\Psi \in C^\infty_c(\mathbb{R}^d)$ be a smooth test function with compact support. Then, under suitable regularity conditions (e.g., Lipschitz continuity of $b$ and $\sigma$), there exists $\alpha > 0$ such that, for any fixed terminal time $T > 0$, there are constants $C > 0$ and $\Delta t^* > 0$ satisfying
\[
\sup_{0 \leq n \leq T/\Delta t} \left| \mathbb{E}[\Psi(x^n)] - \mathbb{E}[\Psi(x_{n\Delta t})] \right| \leq C \Delta t^\alpha,
...
\end{example}
Much like in the continuous case, discussed in Section \ref{sec:linfty}, the convergence to a stationary distribution can often be established via two key assumptions: the Lyapunov condition (Assumption \ref{ass:lyapunov}) and the minorization condition (Assumption
\ref{ass:minorization}). In practice, establishing the minorization condition is often relatively straightforward, whereas verifying the Lyapunov condition requires assumptions on the drift term\cite{mattingly2002ergodicity}. A common choice for the Lyapunov function is $\mathcal{K}(x) = 1 + |x|^n$ for some $n > 0$. Following this, we can apply theorem \ref{thm:exp-conv-linf} to prove exponential convergence of the iterate distribution to the invariant distribution. Once ergodicity and irreducibility of the discretized scheme have been established, we can invoke results (e.g., \cite[Chapter~17]{meyn2012-oq}) to obtain almost sure convergence. Let \(\Psi:\mathcal X\to\mathbb R\) be an observable and define the empirical average
\[
\widehat\Psi_{N_{\rm iter},\Delta t}
:=
\frac{1}{N_{\rm iter}}
\sum_{k=0}^{N_{\rm iter}-1}\Psi(x^k),
\]
where \((x^k)_{k\ge0}\) is the Markov chain generated by the discretized dynamics with timestep \(\Delta t\). Then,
\begin{equation}
\widehat{\Psi}_{N_{\text {iter }}, \Delta t} \xrightarrow[N_{\text {iter }} \rightarrow+\infty]{\text { a.s. }} \int_{\mathcal{X}} \Psi \mathrm{d} \pi_{\Delta t} .
\end{equation}
...
\begin{equation}
\mathbb{E}_{\pi_{\Delta t}}\left[\Pi_{\Delta t} \Psi\left(x^n\right) \Pi_{\Delta t} \Psi\left(x^m\right)\right]=\mathbb{E}_{\pi_{\Delta t}}\left[\Pi_{\Delta t} \Psi\left(x^{|n-m|}\right) \Pi_{\Delta t} \Psi\left(x^0\right)\right],
\end{equation}
for all integers $n,m \geq 0$. To derive expressions for the asymptotic variance of the discretization, similar to that of the continuous SDE above:
\begin{equation}
\varsigma_{\Psi, \Delta t}^2=N_{\mathrm{corr}, \Delta t, \Psi} \varsigma_{\Psi, \mathrm{iid}, \Delta t}^2 \quad \text { where } \varsigma_{\Psi, \mathrm{iid}, \Delta t}^2=\int_{\mathcal{X}}\left(\Pi_{\Delta t} \Psi\right)^2 \mathrm{~d} \pi_{\Delta t},
\end{equation}
where
\[
N_{\mathrm{corr},\Delta t,\Psi}
=
1+2\sum_{k=1}^{\infty}
\frac{
\mathbb E_{\pi_{\Delta t}}
\!\left[
(\Pi_{\Delta t}\Psi)(x^k)
(\Pi_{\Delta t}\Psi)(x^0)
\right]
}{
\varsigma_{\Psi,\mathrm{iid},\Delta t}^{2}
}
\]
is the integrated autocorrelation factor, which quantifies the increase in variance due to temporal correlations. Here, $\Pi_{\Delta t} \Psi=\Psi-\int_{\mathcal{X}} \Psi \mathrm{d} \pi_{\Delta t}$ and $\varsigma_{\Psi,\mathrm{iid},\Delta t}^{2}$ denotes the asymptotic variance associated with independent samples from $\pi_{\Delta t}$.

Furthermore, if we let $\theta_{\mathrm{corr},\Psi}$ denote the corresponding decorrelation timescale in the continuous-time limit, then in the limit of small $\Delta t$, the relation between the decorrelation of discrete and continuous dynamics is
$$\theta_{\mathrm{corr}, \Psi} \simeq N_{\mathrm{corr}, \Delta t, \Psi} \Delta t.$$
...
In practice, we are interested in using $\widehat{\Psi}_{N_{\text{iter}},\Delta t}$ to approximate
\[
\mathbb{E}_\pi(\Psi) = \int_\chi \Psi \, d\pi.
\]
The total error can be decomposed into two contributions:
\begin{equation} \label{eq:statistic_bias}
    \widehat{\Psi}_{N_{\text{iter}},\Delta t} - \mathbb{E}_\pi(\Psi) = \underbrace{\left( \widehat{\Psi}_{N_{\text{iter}},\Delta t} - \mathbb{E}_{\pi_{\Delta t}}(\Psi) \right)}_{\text{Statistical Error}} + \underbrace{\left( \mathbb{E}_{\pi_{\Delta t}}(\Psi) - \mathbb{E}_\pi(\Psi) \right)}_{\text{Bias}}.
\end{equation}

\paragraph{Statistical Error.}
Once the asymptotic regime is attained, the statistical error scales like $\mathcal{O}(1/\sqrt{N_{\text{iter}}})$. More precisely, for asymptotic variance $\varsigma_{\Psi,\Delta t}^2$
\[
\lim_{\Delta t \to 0} \Delta t \, \varsigma_{\Psi,\Delta t}^2 = \varsigma_\Psi^2,
\]
where
\[
\varsigma_\Psi^2 = \lim_{t \to +\infty} t \, \mathrm{Var}_\pi(\hat{\Psi}_t), \quad \varsigma_{\Psi,\Delta t}^2 = \lim_{N_{\text{iter}} \to +\infty} N_{\text{iter}} \, \mathrm{Var}_{\pi_{\Delta t}}\left( \widehat{\Psi}_{N_{\text{iter}},\Delta t} \right).
\]
...
\begin{equation} \label{bias_error}
    \int_{\chi} \Psi \, d\pi_{\Delta t} = \int_{\chi} \Psi \, d\pi + \Delta t^p \int_{\chi} \Psi f \, d\pi + \mathcal{O}(\Delta t^{p+1}),
\end{equation}
...
\[
P_{\Delta t} \Psi = \Psi + \Delta t \mathcal{A}_1 \Psi + \Delta t^2 \mathcal{A}_2 \Psi + \cdots + \Delta t^{p+1} \mathcal{A}_{p+1} \Psi + \Delta t^{p+2} r_{\Psi, \Delta t},
\]
for test functions $\Psi$ in a suitable space $\mathcal{S}$. The operators $\mathcal{A}_k$ are typically differential operators determined by the structure of the numerical scheme, and the order $p$ corresponds to the weak accuracy of the integrator. Typically:
\begin{itemize}
    \item $\mathcal{A}_k$ are identified by expanding $\Psi(x^{n+1})$ around $\Psi(x^n)$ using the dynamics of the scheme.
...
Define:
\[
\mathcal{S} = \left\{ \Psi \in C^\infty(\mathcal{X}) \,\middle|\, \text{ for all } k \in \mathbb{N}^d, \text{ there exists } m \in \mathbb{N} \text{ such that } \partial^k \Psi \in L^\infty_{\mathcal{K}_m}(\mathcal{X}) \right\},
\]
where $L^\infty_{\mathcal{K}_m}$ denotes the space of functions satisfying $\|\partial^k \Psi / \mathcal{K}_m\|_{L^\infty} < \infty$.
...
\begin{theorem}[Error estimates on the invariant measure, \texorpdfstring{\cite[Theorem~3.3]{Lelievre2016-hx}}{}]
    ...
    \begin{equation} \label{A_expansion}
        P_{\Delta t} \Psi = \Psi + \Delta t \mathcal{A}_1 \Psi + \cdots + \Delta t^{p+1} \mathcal{A}_{p+1} \Psi + \Delta t^{p+2} r_{\Psi,\Delta t},
    \end{equation}
    for any $\Psi \in \mathcal{S}$, with some $p \in \mathbb{N}$. Assume that there exist constants $K > 0$, $m \in \mathbb{N}$, and $\Delta t^* > 0$ such that the remainder is uniformly bounded:
    \[
    \| r_{\Psi,\Delta t} \|_{L^\infty(\mathcal{K}_m)} \leq K, \quad \text{for all } \Delta t \leq \Delta t^*.
    \]
    ...
    \begin{enumerate}
        ...
        \begin{equation} \label{Ak_mean_zero}
            \int_\mathcal{X} \mathcal{A}_k \Psi \, d\pi = 0 \quad \text{for all } \Psi \in \mathcal{S};
        \end{equation}
    ...
    \begin{equation} \label{bias_error_estimate}
        \int_\mathcal{X} \Psi \, d\pi_{\Delta t} = \int_\mathcal{X} \Psi \, d\pi + \Delta t^p \int_\mathcal{X} \Psi f_{p+1} \, d\pi + \Delta t^{p+1} R_{\Psi,\Delta t},
    \end{equation}
    with $|R_{\Psi,\Delta t}| \leq L$, and where the bias function $f_{p+1} \in \mathcal{S}_0$ is given by
    \[
...
This chapter established the stochastic and numerical background required for the study of Langevin-based sampling methods. We
introduced stochastic differential equations, the It\^{o} calculus framework, and numerical discretization techniques including
Euler--Maruyama and splitting methods. We also reviewed notions of weak and strong convergence and conditions ensuring ergodicity of
the associated Markov processes.

These results will be used throughout the remainder of the thesis to analyze the behaviour of discretized Langevin dynamics and the
sampling schemes derived from them. The next chapter introduces the specific models and algorithms that form the focus of the analysis
in Part II.
\begin{comment}
    \begin{proof} [Outline of proof]
...
Note that  $G(x) = \sum_i \sigma(y_i A_i^\top x)$  satisfies condition (iv) for any function $\sigma$ with bounded derivatives, which includes the logistic loss and smooth versions of the unit.  The least-squares loss $G(x)=\norm{Ax-y}^2$ does not satisfy condition
(iv) on the boundedness of $\nabla G$. A common alternative is the Huber loss, which replaces the quadratic loss with a function that
is quadratic near zero and linear for large residuals. Writing the residual as $r = Ax - y$, the Huber loss is defined by
\[
L_\delta(r) =
\begin{cases}
...
\end{cases}
\]
where $\delta>0$ controls the transition between quadratic and linear behaviour. However, in our numerical computations, the method behaves well with the quadratic loss so assumption (iv) is likely a limitation of our theoretical analysis rather than the method itself.
...
\end{equation}
A classic approach to sampling from the distribution $\rho$ is via Langevin Dynamics:
\begin{equation}
...
$$
where $G(x) = \frac12\norm{Ax-y}^2$
for some matrix $A\in \real^{m\times d}$ and data vector $y\in \real^m$. Proximal-based Langevin methods and related splitting schemes have become standard tools for sampling from distributions with non-smooth priors, particularly in imaging and inverse problems \cite{pereyra2016proximal, durmus2018efficient, proxalgs}. An interesting approach that  pre-dates  proximal-type methods (and introduces no additional smoothing) is the following Gibbs sampler based on a hierarchical model \cite{park2008Bayesian}, which can be seen as the equivalent of the $\eta$-trick for sampling.
...
The biggest drawback of proximal methods, such as MYULA, is that convergence to the target distribution is only guaranteed in the limit $\gamma \xrightarrow[]{} 0$. Additionally, statistical quantities such as modes are not generally preserved under smoothing. On the other hand, the Gibbs sampler approximates the target distribution exactly when working with quadratic cost functionals, but involves computing matrix inverses, making it unsuitable for high-dimensional problems. We thus introduce a sampling method based on the $u,v$-reparameterization we introduced earlier in section \ref{subsec: hada}. Unlike in the case with smoothing-based methods we no longer need to consider the approximation error. More precisely, by taking the distribution (\ref{target}), the reparametrization and change of variables as seen in (\ref{quad_form_l1}-\ref{uv_form_l1}), we obtain the following probability distribution:
\begin{align}
\pi(u,v)=\frac{1}{Z_\pi} \prod_{i=1}^d  u_i\, \exp\pp{-\beta\, \pp{\frac12\lambda \|(u,v)\|^2+G(u\odot v)} },\label{pi}
\end{align}
over $(u,v)\in \mathcal X= \real_+^d\times \mathbb {R}^d$ for normalisation constant $Z_\pi$ and $\norm{(u,v)}^2=\norm{u}^2+\norm{v}^2$. The Hadamard  product $x=u\odot v$ has the same distribution $\rho$ defined in \eqref{target}. This is formalized in the following statement.





\begin{theorem}\label{thm:overparam}If $(u,v)\sim \pi$ as defined in \eqref{pi}, then $x=u \odot v\sim\rho$ as defined in \eqref{target}. In particular, for any smooth $\phi:\mathbb{R}^d \xrightarrow[]{} \mathbb{R}$,
...
\chapter{Analysis of Hadamard-Langevin Dynamics} \label{chp:analysis_hada_lang}
We begin by introducing the notation used throughout this section. Let $d\in\mathbb N$ denote the dimension. We consider stochastic processes $(u,v)_t\in \mathbb R_+^d \times \mathbb R^d$, and write $x_t = u_t \odot v_t \in \mathbb R^d$.

We denote by $\pi$ the target probability distribution on $\mathbb R^d$, given (formally) by
\[
\pi(x) \propto \exp(-\beta H(x)),
\]
where $\beta>0$ is the inverse temperature parameter, and $H:\mathbb R^d \to \mathbb R$ is the potential function associated with the model.

For notational simplicity, when no ambiguity arises, we occasionally omit the time index $t$.
...
where $W_t^1$ and $W_t^2$ are independent $d$-dimensional standard Brownian motions. We can reparametrize into Cartesian coordinates to obtain the system
...
\end{equation}
for $\vec x \in \mathbb{R}^{3d}$, $G_{3}(\vec x)= G( (y\odot y + z\odot z)^{1/2} \odot v)$ and an $\mathbb{R}^{3d}$-valued Brownian motion $W^3(t)$. The function $G_{3}$ is locally bounded under Assumption \ref{reg1}, and solutions of (\ref{cyl}) are well defined. We use these as a tool to analyze (\ref{uvproc}). More explicitly, the following theorem shows that solutions of the Cartesian system
(\ref{cyl}) induce solutions of the Hadamard--Langevin system (\ref{uvproc}).

\begin{theorem}[Cartesian reparameterization]
\label{thm:cartesian_reparam}
Suppose Assumption~\ref{reg1} holds. Let
\[
x_t=(y_t,z_t,v_t)
\]
be a solution of the Cartesian system~(\ref{cyl}), and define $u_t=(y_t\odot y_t+z_t\odot z_t)^{1/2}.$ Then the process $(u,v)_t$ satisfies the Hadamard--Langevin system~(\ref{uvproc}). Moreover, if the initial distribution of \(x_0\) is cylindrical, then the law of
\((u,v)_t\) is the cylindrical marginal of the law of \(x_t\).
\end{theorem}

\begin{proof}
Since the noise in~(\ref{cyl}) is additive and non-degenerate, the
process \(x_t\) admits a probability density function absolutely
continuous with respect to Lebesgue measure on \(\mathbb R^{3d}\) for each
\(t>0\). Hence
\[
\mathbb P(u_{i,t}=0)
=
\mathbb P((y_{i,t},z_{i,t})=(0,0))
=
0
\]
for all \(t>0\). We may therefore apply It\^o's formula to
\[
u=(y\odot y+z\odot z)^{1/2},
\]
following~\cite[Proposition~3.21]{Karatzas1991-xr} by approximating the
square-root function with smooth functions and passing to the limit.
...
-\frac{1}{u_t^3}
\odot
(y_t\odot y_t+z_t\odot z_t)
+
\frac{2}{u_t}
\right)dt .
\end{aligned}
\]
Using \(u_t^2=y_t^2+z_t^2\), this simplifies to
\[
du_t
=
-\lambda u_t\,dt
...
\frac{1}{\beta u_t}\,dt
+
\sqrt{\frac{2}{\beta}}\,dW_t,
\]
where
\[
W_t
=
u_t^{-1}\odot (y_t,z_t)\cdot (W_t^{3,y},W_t^{3,z})
\]
is again a Brownian motion by~\cite[Theorem~3.3.16]{Karatzas1991-xr}.
This is precisely the \(u\)-equation in~(\ref{uvproc}). The \(v\)-equation
follows directly from~(\ref{cyl}). Hence \((u,v)_t\) satisfies
\((\ref{uvproc})\).

Finally, by the definition of a cylindrical distribution, if the initial
law of \(x_0\) is cylindrical, then the law of
\[
\big((y_t\odot y_t+z_t\odot z_t)^{1/2},v_t\big)
\]
is the cylindrical marginal of the law of \(x_t\). This proves the claim.
\end{proof}
...
If the initial distribution is cylindrical, the marginal distribution of $((x_t^2+y_t^2)^{1/2},v_t)$ equals the law of the solution $(u,v)_t$ of (\ref{uvproc}).
...
Suppose that Assumption~\ref{reg1} holds. Subject to initial
condition $(u,v)_0=(U_0,V_0)\in \mathcal X=\mathbb R_+^d\times\mathbb R^d$,
there is a unique solution $(u,v)_t$ to~(\ref{uvproc}) such that
\[
u_t>0 \quad \text{almost surely}
\]
and
\[
\mathbb E\Big[\sup_{0\le s\le T}\|(u,v)_s\|^q\Big]<\infty,
\qquad
T>0,\; q\ge 2.
\]
Further, $(u,v)_t$ is strong Feller and irreducible on $\mathcal X$.

The probability density function $p(t,\cdot)$ of $x_t$
(see~(\ref{cyl})) is well-defined. If the initial distribution is
cylindrical, then the marginal distribution of
\[
\big((y_t^2+z_t^2)^{1/2},\,v_t\big)
\]
equals the law of the solution $(u,v)_t$ of~(\ref{uvproc}).
\end{theorem}


\begin{proof}
Under Assumption~\ref{reg1}, $\nabla G_3$ is bounded on $\mathbb R^{3d}$.
Indeed, $\nabla G$ is bounded by Assumption~\ref{reg1}(iv), and the
chain-rule factors $y_i/u_i$ and $z_i/u_i$, where
$u_i=(y_i^2+z_i^2)^{1/2}$, are uniformly bounded by $1$. Hence the drift
\[
-\lambda x-\nabla G_3(x)
\]
of~(\ref{cyl}) is bounded and measurable on $\mathbb R^{3d}$.
Furthermore, the diffusion coefficient
\[
a=(2/\beta)I_{3d}
\]
is constant and uniformly elliptic, so conditions (1.3) and (1.4) of
\cite{Stroock2013-df} hold.
...
-\lambda\|x\|^2+K
\]
follows from Assumption~\ref{reg1}(iii). Applying It\^o's formula to
$\|x_t^{(n)}\|^2$ for the localized equations and using Gronwall's
inequality yields
\[
\mathbb E\Big[\sup_{s\leq t}\|x_s^{(n)}\|^2\Big]
\leq C,
\]
uniformly in the localization level $n$, for each $t>0$. Chebyshev's
inequality then implies the non-explosion condition (1.7) of
\cite[Theorem 10.1.3]{Stroock2013-df}:
\[
\lim_{n\to\infty}
\mathbb P\!\left(
\sup_{s\leq t}\|x_s^{(n)}\|\geq n
\right)
=0.
\]

Consequently, \cite[Corollary 10.1.4]{Stroock2013-df} implies that the
martingale problem associated with $(a,b)$ is well posed, that $x_t$ is
strong Feller, and that $x_t$ admits a probability density function
absolutely continuous with respect to Lebesgue measure on
$\mathbb R^{3d}$ for each $t>0$. Higher moments satisfy
\[
\mathbb E\Big[\sup_{s\leq t}\|x_s\|^q\Big]
<\infty,
\]
which follows analogously by applying It\^o's formula to
$(\|x_t\|^2)^{q/2}$. Irreducibility of $x_t$ follows from the
non-degenerate additive noise by standard arguments
\cite[Section~6]{Bellet2006-pq}.

\begin{comment}
    The generator for~(\ref{uvproc}) has singular coefficients on a
half-space and regularity of the density does not follow directly from
standard SDE theory. We therefore show that the $u$-equation
in~(\ref{uvproc}) arises from the Cartesian equation~(\ref{cyl}) and
that the process $(u,v)_t$ inherits properties from $x_t$.

Since the noise in~(\ref{cyl}) is additive and non-degenerate, the
process $x_t$ has a probability density function absolutely continuous
with respect to Lebesgue measure on $\mathbb R^{3d}$ for each $t>0$
by \cite[Corollary 10.1.4]{Stroock2013-df}. Hence
\[
\mathbb P(u_{i,t}=0)
=
\mathbb P((y_{i,t},z_{i,t})=(0,0))
...
u=(y\odot y+z\odot z)^{1/2},
\]
following \cite[Proposition~3.21]{Bellet2006-pq} by approximating the
square-root function with smooth functions and passing to the limit.
...
u_t^{-1}\odot (y_t,z_t)\cdot(W_t^{3,y},W_t^{3,z})
\]
is again a Brownian motion by
\cite[Theorem~3.3.16]{Karatzas1991-xr}. This is precisely the
$u$-equation in~(\ref{uvproc}). The $v$-equation follows directly
from~(\ref{cyl}).
\end{comment}


By Theorem~\ref{thm:cartesian_reparam},
the process
\[
u_t=(y_t\odot y_t+z_t\odot z_t)^{1/2}
\]
induces a solution \((u,v)_t\) of~(\ref{uvproc}).

Since \(x_t\) is well posed, strong Feller, irreducible, and satisfies
the moment bounds established above, it follows that \((u,v)_t\) is
well posed on \(\mathcal X=\mathbb R_+^d\times\mathbb R^d\), is strong
Feller, irreducible, and satisfies the corresponding moment bounds.
The cylindrical marginal statement follows from
Theorem~\ref{thm:cartesian_reparam}.
...
\cite[Corollary 10.1.4]{Stroock2013-df} gives that $\vec x_t$ is strong Feller and has a well-defined probability density function. To apply this result, we consider SDEs with mollified (compactly supported) drift and diffusion coefficients, which are smooth and globally Lipschitz. In addition, one requires the condition
\[
\lim_{n\to\infty}\mathbb P\!\left(\sup_{0\le t\le T}\|\vec x^{(n)}_t\|\ge n\right)=0,
\]
where $\vec x^{(n)}$ denotes the solution of the mollified system. While global Lipschitzness ensures that each mollified SDE is well posed, this condition does not follow directly. Under Assumption~\ref{reg1}, the drift of the Cartesian system has at most linear growth, and one expects the above condition to follow from a suitable Lyapunov estimate. We do not establish this estimate in full generality here, and regard it as an additional technical step required for the application of \cite[Theorem 10.1.3]{Stroock2013-df}.

Due to the non-degenerate and additive noise, $\vec x_t$ is irreducible by standard arguments~\cite[Section 6]{Bellet2006-pq}


 The generator for (\ref{uvproc}) has singular coefficients on a half space and regularity of the density does not follow directly from standard theories; we show that the $u$-equation in (\ref{uvproc}) arises from the solution $\vec x_t$ of (\ref{cyl}) and that the process $(u,v)_t$ inherits properties from $\vec x_t$. Assuming that $u_t\ne 0$, we may apply the Ito formula to  $u_t=(y_t\odot y_t+z_t\odot z_t)^{1/2}$ and obtain, for independent $d$-dimensional brownian motions $dW_t^{3,y}, dW_t^{3,z}$,
...
\end{equation}
which, if we set $\theta = 2/\sqrt{2/\beta}$, $\kappa = 2\lambda$ and $\mu = \frac{2}{\beta \lambda}$, are precisely the $u_t$ dynamics (\ref{uvproc}) with $G \equiv 0$. Note, that in this case, $2\kappa \mu = \theta^2$. The Ornstein-Uhlenbeck (OU) process is given by
$$
d\hat v_t =-\lambda\, \hat v_t\, dt + \sqrt{\frac{2}{\beta}}\,dW_t^2,
$$
which is precisely our $v$ process with $G \equiv 0$. As such, in order to show well-posedness of our SDE, we consider \eqref{uvproc} as perturbed CIR and OU processes. Using Girsanov' Theorem we will be able to translate the analysis of the latter via absorbing the data term $G$. In the following, let $\hat z_t \eqdef (\hat u,\hat v)_t$ satisfy
\begin{align}\label{potential_free_sde_z}
d \hat z_t = \biggl[-\lambda \hat z_t + \binom{1/(\beta \hat u_t)}{0} \biggr]\,dt + \sqrt{\frac{2}{\beta}} \,d  W_t,
\end{align}
...
\begin{lemma}[Novikov condition]\label{lem:novikov}
    ...
    $$
    There exists a weak solution $(u,v)_t$ of (\ref{uvproc}) that is unique in law. Additionally, $u_t>0$ almost surely.
\end{lemma}
\begin{proof}
...
Given  $\alpha>0$ and  $\hat{y}_t \eqdef\exp\left(\frac{\alpha}{2}\norm{(\hat{u}_t,\hat{v}_t)}^2 \right)$,
\begin{align*}
    d\hat y_t &= \pa{-\lambda \alpha  + \frac{\alpha^2}{\beta}} \,\norm{\hat z_t}^2\hat y_t\, dt + \pa{\frac{3\alpha d}{\beta}}\, \hat y_t\, dt + \sqrt{\frac{2}{\beta}} \alpha \hat y_t  \pa{\hat u_t^\top d W^1_t + \hat v_t^\top d W^2_t}.
\end{align*}
%\begin{align*}
...
Letting $\alpha = 2\epsilon \gamma \cinfty^2 $, the condition on $\alpha$ is satisfied if $\epsilon  <\beta\lambda/(2\gamma \cinfty^2)$. Since $\gamma>0$ and $B<\infty$, such an $\varepsilon$ exists. Substituting for $\alpha$,
$
...
$$
Thus, Novikov's condition is satisfied. We can therefore apply Girsanov's theorem. Let

\[
M_T
=
\exp\!\left(
\int_0^T b(\hat z_t)\cdot dW_t
-
\frac12
\int_0^T \|b(\hat z_t)\|^2dt
\right).
\]

Since Novikov's condition holds, \(M_T\) is a martingale.
Following the construction of Section \ref{sec:bm_girsanov}, define a new probability measure \(\mathbb Q\) on
\(\mathcal F_T\) by
\[
...
\widetilde W_t
=
W_t-\int_0^t b(\hat z_s)\,ds
\]
is a Brownian motion under \(\mathbb Q\).
Substituting this expression into (\ref{potential_free_sde_z}) yields
\[
d\hat z_t
=
\Bigl[-\lambda \hat z_t
...
+b(\hat z_t)\Bigr]dt
+
\sqrt{\frac{2}{\beta}}\,d\widetilde W_t,
\]

which is precisely the Hadamard--Langevin system (\ref{uvproc}). Hence (\ref{uvproc}) admits a weak solution. Since \(\mathbb Q\) is equivalent to \(\mathbb P\), uniqueness in
law follows from the corresponding uniqueness result for the
reference equation (\ref{potential_free_sde_z}) (see~\cite[Corollary 5.14]{Karatzas1991-xr}).
...
Thus, via H\"{o}lder's inequality,
 ...
 completes the proof.
\end{proof}
As a corollary, we prove the following moment bounds on $1/u_{i,t}^r$ for $r<2$. The density of the invariant measure $\pi$ behaves like $u_i$ near $u_i=0$, and we expect $\mathbb{E}\bp{1/u_{1,i}^2}$ to be infinite. 
...
2. Let $\mathcal R=\{ (u_{i,t}\wedge u_{i,s})<\Delta\}$, i.e. the region where $\frac{1}{u_{i,s}u_{i,t}}$ might blow up. Then, by Boole's inequality $\prob(\mathcal R) \le  \prob\pp{u_{i,t}<\Delta} +  \prob\pp{u_{i,s}<\Delta}$.  By Chebyshev's inequality, $\prob\pp{u_{i,t}<\Delta}\le  \Delta^{2-\epsilon}\, \mean{u_{i,t}^{\epsilon-2}}$ for $\epsilon>0$. Hence, using Corollary \ref{cor}, for a constant $c$, $\prob\pp{\mathcal R}\le c \,\Delta^{2-\epsilon}$. Now, split the expectation with respect to $\mathcal R$ as 
 ...
 \end{align*}
 Choose $p'=2+\epsilon$ and its conjugate exponent $p<2$ so that  $\mean{u_{i,t}^{-p}}<\infty$. As $\prob(\mathcal{R})\le c\, \Delta^{2-\epsilon}$ for some $c$, 
...
In the last line, we exploit  $u_i(s),u_i(t) \geq \Delta$ outside $\mathcal R$.
...
\begin{align*}
    ...
    &\quad -\frac{\beta}{4\tstep} \norm{   (1+\tstep\,\lambda) v^{k+1}  - v^{k}  + \tstep\, u^k\odot \nabla G(u^k\odot v^k)) ) }^2\\
    &\quad + \sum_{i=1}^d\log\pa{ 1+\tstep\,\lambda + \frac{\tstep}{\beta  (u^{k+1}_i)^2}}.
\end{align*}
\end{lemma}
\begin{proof}
Let
...
$$
Substituting $t = u^{k+1}_i$ in the Jacobian factor gives $\prod_{i=1}^d\left(1+\Delta t \lambda+\frac{\Delta t}{\beta\left(u_i^{k+1}\right)^2}\right)$. The density of $P_{1,v}(v)$ is
...
\end{itemize}
\end{assumption}
The boundedness of $\frac{\rho_0}{\pi}$ is used only to guarantee the finiteness of the moments of the initial condition required in the proof below. A suitable moment assumption on $\rho_0$ would therefore be sufficient. For $t_k=k\,\tstep$, let  $e_1(t_k) \eqdef u_{t_k} - u^k$ and $e_2(t_k) \eqdef v_{t_k} - v^k$, and $e(t_k) \eqdef (e_1(t_k), e_2(t_k)) $. Our main convergence result is as follows.
...
To overcome this, we derive inverse-moment bounds of order up to but excluding 2 (we will make use of Corollary (\ref{cor}) and Proposition (\ref{timeregbnd}) in our proof). As a result, in the case where $G\equiv 0$, our proof shows that the discretized CIR process with parameters $2\kappa\mu =\theta^2$  satisfies the strong error bound $$\EE\biggl[\sup_{0\leq t_k \leq T} \norm{ u_{t_k} - u^k}\biggr] = \Oo(\deltat^{\frac12-\epsilon}),$$  for any $\epsilon>0$. We remark that similar ideas and error bounds were applied in \cite{hutzenthaler2014strong}.
...
The proof of \ref{thm:num_conv}  is a consequence of Propositions \ref{prop:error_discrete} and \ref{prop:error_discrete_part2} below. We first make two simplifying assumptions, \ref{ass:ball_R},\ref{ass:2}, in order to derive convergence in \ref{prop:error_discrete}. We then show in Proposition \ref{prop:error_discrete_part2} that these two assumptions hold under our main assumption (\ref{reg3}). 
 %
 We remark that while we manage to show $\tstep^{1/2-\epsilon}$ rate for a stopped process (restricted to the ball of radius $R$) in Proposition \ref{prop:error_under_bounded_ball}, our main convergence result \ref{thm:num_conv} does not provide a rate because our error bound has an exponential dependence on $C_R$ and hence $R$.
...
$$
\end{assumption}
Note that Assumption \ref{ass:2} is the discrete counterpart to the $p$th-moment bounds (\ref{uv_moment}) on $(u,v)_t$. We  show in Proposition \ref{prop:error_discrete_part2} that Assumption \ref{ass:2} is indeed satisfied for our discretized process.
...
\begin{proposition}\label{prop:error_under_bounded_ball}
  Let the assumptions of Proposition \ref{prop:error_discrete} hold and $\deltat \leq \frac{1}{2C_R}$.  
    For all $\epsilon>0$, 
    \begin{equation} \label{stopping_hypo}
...
\end{align*}
Define $c_1 = \pa{1+ 2\,C_R\, \deltat }/ \pa{1 + 2\lambda\, \deltat - \,C_R\,\tstep}$ and $c_2 = 2/ \pa{1 + 2\,\lambda\, \deltat -  C_R\,\tstep}$. Note, that for sufficiently small \(\Delta t\), the denominator
\(1+2\lambda\Delta t-C_R\Delta t\) is positive.
%\begin{align*}
...
% \end{align*}
\begin{align*}
    E_{k+1}^2 &\leq c_1 E_k^2 + c_2 E_{k+1} R_k \leq c_1^{k+1} E_{0}^2 + c_2 \sum_{j=0}^{k} c_1^j  E_{k+1-j} R_{k-j}
 \end{align*}
Let $M=\left\lfloor\frac{T}{\Delta t}\right\rfloor$,
\[
N:=M\wedge k_R,
\qquad
A:=\sup_{0\leq m\leq N}E_m .
\]
Since \(E_0=0\), we have
\[
A^2=\sup_{0\leq k\leq N-1}E_{k+1}^2 .
\]
The recursive estimate gives, for \(0\leq k\leq N-1\),
\[
E_{k+1}^2
\leq
c_2 A\sum_{j=0}^{k}c_1^jR_{k-j}.
\]
Taking the supremum over \(0\leq k\leq N-1\), we obtain
\[
A^2
\leq
c_2 A
\sup_{0\leq k\leq N-1}
\sum_{j=0}^{k}c_1^jR_{k-j}.
\]
If \(A=0\), the desired estimate is trivial. Otherwise, dividing by \(A\) yields
\[
A
\leq
c_2
...
\sum_{j=0}^{k}c_1^jR_{k-j}.
\]
Writing \(\ell=k-j\), we have
\[
\sum_{j=0}^{k}c_1^jR_{k-j}
=
\sum_{\ell=0}^{k}c_1^{k-\ell}R_\ell .
\]
%Since \(c_1=1+O(\Delta t)\) and \(M\Delta t\leq T\), there exists a constant
%\(C_T>0\), independent of \(\Delta t\), such that
%\[
%\sup_{0\leq m\leq M}c_1^m\leq C_T .
%\]
Since
\[
c_1=\frac{1+2C_R\Delta t}{1+2\lambda\Delta t-C_R\Delta t}
=1+\mathcal O(\Delta t),
\]
there exists a constant \(C>0\), independent of \(\Delta t\), such that
\(c_1\le 1+C\Delta t\) for sufficiently small \(\Delta t\). Hence, using
\(1+x\le e^x\),
\[
c_1^m
\le (1+C\Delta t)^m
...
\le e^{CT},
\]
since \(m\le M\) and \(M\Delta t\le T\). Therefore,
\[
\sup_{0\le m\le M} c_1^m\le C_T,
\]
where \(C_T=e^{CT}\) is independent of \(\Delta t\).
Therefore, since $k \leq N \leq M$, we have $k-\ell \leq M$, and therefore $c_1^{k-\ell} \leq  C_T,$ and so
\[
\sum_{\ell=0}^{k}c_1^{k-\ell}R_\ell
\leq
C_T\sum_{\ell=0}^{M-1}R_\ell .
\]
Hence
\[
\mathbb E[A]
\leq
c_2 C_T
\sum_{\ell=0}^{M-1}\mathbb E[R_\ell].
\]


%\[
%\mathbb E\left[
%\sup_{0\leq m\leq M\wedge\theta_R}E_m
%\right]
%=
%\mathbb E[A]
%\leq
...
%\[
%\mathbb E[R_\ell]\leq C_R\Delta t^{3/2-\varepsilon},
%\]
%uniformly in \(\ell\), and \(M\leq T/\Delta t\), we obtain
%\[
%\mathbb E\left[
%\sup_{0\leq m\leq M\wedge\theta_R}E_m
%\right]
%\leq
%C_R T \Delta t^{1/2-\varepsilon}.
%\]
...
%Also using the geometric sum formula and the inequality $e^z \geq 1 + z$ for $z\geq 0$, we get
 ...
 % OLDOLD $$\sum_{j=0}^{M-1} c_1^j \leq \frac{c_1^M - 1}{\alpha - 1} = \frac{\exp(2\deltat M C_R)}{4\deltat C_R + 4 \deltat^2 C_R^2}.$$ \fc{shouldn't we have $4\deltat M C_R$ in the exp(...)?}
...
Thus, uniformly in \(k\),
\[
\mathbb E[R_k]\leq C\Delta t^{3/2-\varepsilon}.
\]
Since \(M\le T/\Delta t\), it follows that
\[\mathbb E[A]
\le
C(\lambda,C_R,d,\varepsilon,T)
\Delta t^{1/2-\varepsilon}.
\]
%\[
%\mathbb E\left[
%\sup_{0\le m\le M\wedge k_R}E_m
%\right] = \mathbb E[A]
%\le
%C(\lambda,C_R,d,\varepsilon,T)
%\Delta t^{1/2-\varepsilon},
%\]
Finally, the preceding argument was carried out for all grid indices
satisfying \(t_k\le \theta_R\). Therefore
\[
\mathbb E\left[
\sup_{0\le t_k\le T}
\|e(t_k\wedge\theta_R)\|
\right]
\le
C(\lambda,C_R,d,\varepsilon,T)\Delta t^{1/2-\varepsilon},
\]
as required. This proves the desired bound on the grid up to the stopping time \(\theta_R\), and hence
establishes (\ref{stopping_hypo}).
...
where $M = \lfloor \frac{T}{\Delta t} \rfloor$. For the last two terms, observe that
  $\EE\sum_{j=0}^{M-1} \norm{\deltaw_j}^p \lesssim M (\deltat)^{p/2}$ and that, by the Burkholder--Davis--Gundy inequality \cite{10.1214/aop/1176997023}, and together with the fact that $f_1,f_2$ have at most linear growth with respect to $\|(u,v)\|$, we have \begin{align*}
...
The first assumption, (\ref{hoheh}), which requires a geometric rate of convergence uniform in the timestep $\tstep$, is more challenging. The geometric ergodicity presented in Theorem \ref{num_erg} is geometric in the number of time steps, but the estimate degrades as the time step is decreased and is not sufficient. Despite this limitation, (\ref{hoheh}) is a natural assumption to make and numerical evidence suggests that the discretized invariant measure $\pi_{\tstep}$ should converge to the true invariant measure $\pi$ as $\Delta t \to 0$. See Subsection \ref{subsec: 1d_experiment}, for example.
...
for $R_n \sim N(0,I_{3d})$. However, due to the singular drift term $1/u_t$, the Euler--Maruyama scheme does not preserve positivity and may produce iterates near or across the singularity at $u=0$, leading to large numerical errors and poor approximation of the long-time dynamics.
...
\end{aligned}
\end{equation}
However, this should perform worse than BOB due to asymmetry creating $\mathcal{O}(h)$ terms in BCH expansion. We also confirm this behaviour in numerical experiments, which we do not present here.
\begin{comment}
...
\end{equation}
\end{comment}
Motivated by the Leimkuhler--Matthews method for stochastic gradient systems
\cite{LeimkuhlerMatthewsTretyakov2014}, we consider the following
Pseudo-Euler--Maruyama (PEM) scheme:
...
\,(R_n+R_{n+1}),
\qquad
R_n,R_{n+1}\sim N(0,I_{3d}).
\]

Numerical experiments suggest that this scheme approximates stationary averages
considerably more accurately than the standard Euler--Maruyama discretization and
exhibits approximately second-order convergence of these averages with respect to the
timestep $h$. Similar second-order accuracy for stationary averages was established for the Leimkuhler--Matthews method in \cite{LeimkuhlerMatthewsTretyakov2014}.
...
Throughout this chapter, several notions of error are relevant. For optimization algorithms, quantities such as the objective value, gradient norm, or distance to the optimum are the primary measures of performance. In contrast, algorithms designed for Bayesian inference aim to approximate the posterior distribution rather than simply produce accurate predictions. Consequently, predictive accuracy alone is not sufficient to assess sampling quality. Moreover, for SDE-based sampling methods, one distinguishes between strong error, which measures pathwise approximation accuracy, and weak error, which measures the accuracy of expectations of test functions. Since many Bayesian inference tasks ultimately require accurate posterior statistics rather than individual sample trajectories, weak error is often the more relevant notion. Accordingly, the experiments in this chapter employ error metrics appropriate to each task. Depending on the problem, these include optimization-related quantities, errors in posterior expectations, effective sample sizes, posterior summary statistics, and predictive performance metrics where appropriate. When predictive metrics are reported for Bayesian methods, they should be interpreted as measures of predictive performance rather than, on their own, measures of posterior approximation quality. These distinctions should be kept in mind when interpreting the numerical results presented in this chapter.

\subsection{1D Regression Problem} \label{subsec: 1d_experiment}
In this section, we compare the sampling methods introduced in Chapters 5 and 6 on Bayesian Lasso regression problems. We consider both one-dimensional and higher-dimensional settings and assess the accuracy of the resulting stationary distributions and sampling efficiency. Unless otherwise stated, the model parameters are chosen as described below. We consider the case where $d=1$, and choose $A=1$, $y=3$, and $\lambda = 2.7$. To estimate the impact of $\Delta t$ for the stationary distribution, for different stepsizes, we run Hadamard Langevin for $T_{burn}=3000$ and then record samples over $T_{rec} = 2500$ total time. In order to counter the Monte-Carlo error (as in (\ref{eq:statistic_bias})), we scale burn-in and sampling iterations as $\mathcal{O}(\Delta t^{-1})$. Furthermore, we scale subsampling frequency as $\mathcal{O}(\Delta t^{-1})$, so that the total number of samples in each chain is constant at 100. Finally, and most importantly, in order to achieve quadratic growth of independent samples, we increase the number of paths at each timestep as $\mathcal{O}(\Delta t^{-2})$. Even so, the best performing method was still dominated by Monte-Carlo error, as seen in (\ref{fig:error_1d}).
\begin{figure}[htb]
    \centering
    ...
    \label{fig:error_1d}
\end{figure}
We carry out the experiment with $\phi(x) = x^2$. Figure \ref{fig:error_1d} shows the error between the mean of  $\phi(u\odot v)$  evaluated via these samples compared with the integral of the true density $\rho$ (i.e., \eqref{target}) against $\phi$ (we compute this numerically  using \texttt{scipy.integrate}). This procedure is also repeated for Proximal Langevin, as well as the Cartesian discretizations, BOB and PEM. In general, we observe $\Oo(\deltat)$ for Proximal Langevin, Hadamard and BOB. PEM performed significantly better, although due to the Monte-Carlo error, it is difficult to say whether the quadrature error scaled as $\Oo(\deltat)$.


\subsection{20-dimensional regression}
We consider the case of $d=20$, and $A\in\RR^{40\times 20}$ is a random Gaussian matrix of mean zero and variance $\frac{1}{16m}$. We let $\lambda = \frac12 \norm{A^\top y}_\infty$ and $y=A x_0$ where $x_0$ is a 2-sparse vector - the first two components of $x_0$ are nonzero and therefore correspond to the informative dimensions of the regression problem. The remaining eighteen components correspond to noise dimensions. In Figure~\ref{fig:means-dim20}, the informative dimensions are highlighted to distinguish their behaviour from that of the noise dimensions. For Proximal and Hadamard Langevin, BOB and PEM, we run $10^4$ burn-in iterations before recording $10^5$  samples,  while for Gibbs (due to its slower running time and faster convergence), we record $10^4$ samples after 10 burn-in samples. To assess the convergence of each scheme, we evaluate the effective sample size (ESS) which we computed using the \texttt{arviz} package \cite{arviz_2019}. The running sample means for each of the 20 dimensions are shown in Figure \ref{fig:means-dim20}. Figure \ref{fig:means-dim20} also shows the ESS in each dimension. Note that Gibbs has the highest ESS, followed by PEM, Hadamard and BOB. In general, for these low-dimensional settings, the  Gibbs sampler displays fast mean convergence behavior, and the Hadamard and splitting method samplers also display a similar pattern (although less sharp than the Gibbs sampler).
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{IMG/20d/figure_7_2_20d.pdf}
    \caption{(a)--(e) Running averages of iterates for the Prox-L1, UV Hadamard, BOB, PEM and Gibbs samplers, respectively. The highlighted curves correspond to the two informative dimensions of the true parameter vector. (f) Effective sample sizes of the covariates for each sampling method.}
    \label{fig:means-dim20}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{(left) Example synthetic dataset showing the informative dimensions and the decision boundary; (right) Typical MAP estimate obtained with a larger value of $\lambda$. The nonzero coefficients correspond primarily to the informative dimensions.}
    \label{fig:db3d}
\end{figure}
...
\end{equation}
where $A \in \mathbb{R}^{200 \times 100}$ is the design matrix containing 200 training samples and $y \in \{-1, 1\}^{200}$ are the associated class labels.

It is important to note that the predictive metrics reported in this section
(classification accuracy and ROC/AUC score) measure out-of-sample predictive
performance rather than the quality of the posterior approximation itself.
A method may achieve high predictive accuracy while providing only a point
estimate of the posterior distribution, whereas Bayesian samplers are designed
to approximate the full posterior and quantify uncertainty. Consequently,
differences in predictive accuracy should not be interpreted as direct measures
of sampling quality.
...
%The results for all experiments are visualized in Tables \ref{tab:train_test_accuracy_binary}--\ref{tab:train_test_accuracy_binary_small_corr}.
Interestingly, the deterministic Lasso solver achieved the highest median
classification accuracy in all four experiments. However, these predictive
metrics should be interpreted with care, since classification accuracy and
ROC/AUC assess predictive performance rather than the quality of posterior
approximation. The Bayesian methods additionally provide uncertainty
quantification by approximating the posterior distribution, which is not
captured by these predictive metrics alone. We therefore interpret these
results primarily as a comparison of predictive performance rather than a
direct comparison of sampling quality.
\begin{table}[h!]
    \centering
    \begin{tabular}{l c c}
        \hline
...
\includegraphics[width=0.19\linewidth]{IMG/haar2d/results-5/uv_mean.pdf}
&\includegraphics[width=0.21\linewidth]{IMG/haar2d/results-5/uv_percentile.pdf}
&
\includegraphics[width=0.19\linewidth]{IMG/haar2d/results-5/prox_mean.pdf}
&
\includegraphics[width=0.21\linewidth]{IMG/haar2d/results-5/prox_percentile.pdf}
\\
\rotatebox{90}{\hspace{2em}$\beta=100$}&
\includegraphics[width=0.19\linewidth]{IMG/haar2d/results-100/uv_mean.pdf}
&\includegraphics[width=0.21\linewidth]{IMG/haar2d/results-100/uv_percentile.pdf}
&
\includegraphics[width=0.19\linewidth]{IMG/haar2d/results-100/prox_mean.pdf}
&
\includegraphics[width=0.21\linewidth]{IMG/haar2d/results-100/prox_percentile.pdf}
\\
\rotatebox{90}{\hspace{2em}$\beta=500$}&
\includegraphics[width=0.19\linewidth]{IMG/haar2d/results-500/uv_mean.pdf}
&\includegraphics[width=0.21\linewidth]{IMG/haar2d/results-500/uv_percentile.pdf}
&
\includegraphics[width=0.19\linewidth]{IMG/haar2d/results-500/prox_mean.pdf}
&
\includegraphics[width=0.21\linewidth]{IMG/haar2d/results-500/prox_percentile.pdf}
\end{tabular}    
...
\part{Hyperparameter Optimization} \label{part:HOAG}

\chapter{\texttt{Stocbio}, \texttt{stocHOAG} and the conditioning issue} \label{chp: stoc_cond}
\section{Motivation}
Many sparse optimisation problems arising in statistics, machine learning and inverse problems are naturally formulated as $\ell_1$-regularised optimisation problems of the form
\[
    \min_x F(x,\lambda)+\lambda\|x\|_1,
\]
where the regularisation parameter $\lambda$ controls the trade-off between data fidelity and sparsity. Determining an appropriate value of $\lambda$ is often crucial for obtaining good solutions and may naturally be formulated as a bilevel optimisation problem, with $\lambda$ acting as the upper-level variable.

The Hadamard reparameterisation developed in the preceding chapters transforms a broad class of $\ell_1$-regularised optimisation problems into equivalent smooth optimisation problems posed in an expanded parameter space. This reformulation makes such problems amenable to gradient-based optimisation techniques, suggesting the possibility of applying stochastic bilevel optimisation methods to automatically determine suitable regularisation parameters. Motivated by this connection, the present chapter investigates stochastic bilevel optimisation algorithms, with particular emphasis on simplifying parameter selection and understanding the influence of the conditioning of the inner optimisation problem.
\section{Introduction}
In the field of smooth bilevel optimization, existing deterministic algorithms, based on implicit differentiation of the inner problem, such as HOAG \cite{pedregosa2016hyperparameter} and BA \cite{ghadimi2018approximation}, have been effectively used to solve problems in areas such as hyperparameter optimization. These algorithms, however, exhibit a high per-iteration complexity, in large part due to the inversion of a matrix, that takes place during the implicit differentiation stage. To tackle this issue, stochastic algorithms have been developed. Here, stochasticity is introduced in large part to invert the Hessian in sub-$\mathcal{O}(n^3)$ time. We will be investigating one such algorithm in partiular - \texttt{stocBio} \cite{ji2021bilevel}. This algorithm, albeit having one of the fastest convergence rates among comparable algorithms, requires the tuning of a large number of parameters. Furthermore, \texttt{stocBio} suffers from a sensitivity to the conditioning of the inner problem, much like most other bilevel algorithms. In this chapter, we first review the \texttt{stocBio} algorithm and its stochastic Neumann series approximation for the inverse Hessian. We then introduce a simplified variant, termed \texttt{stocHOAG}, whose parameter schedule is considerably easier to tune while retaining comparable convergence guarantees.

\subsection{Problem Statement \& Assumptions}
We begin by introducing the bilevel optimisation problem considered throughout this chapter together with the assumptions required for the subsequent convergence analysis. For a bounded domain $\mathcal D \subset \mathbb{R}^r$, we wish to find $\lambda \in \mathcal{D}$ and $\hat{x}(\lambda) \in \mathbb{R}^n$, such that
\begin{align} 
...
\begin{align*}
    ...
    \mathbb{E}_{\xi}[\tilde{C}(x,\lambda,\xi)] &= C(x,\lambda),
\end{align*}
where $\zeta$ and $\xi$ are drawn uniformly at random from their respective datasets. Let us first make some convexity assumptions. We first state the standard assumptions inherited from the stochastic bilevel optimisation literature before introducing one additional assumption that will be required later.
...
Under these assumptions, the stochastic estimators admit uniformly bounded variances:
...
\subsection{stocBio and \texttt{stocHOAG}}
...
\begin{itemize}
    ...
    \end{itemize}
Note that the parameters for \texttt{stocBio} are set up in a global manner - at any point during the runtime these are fixed. The Neumann series and its sampling will be explained in section \ref{neumann_series}. The algorithm is described in \ref{alg:stocbio}.
\end{comment}
...
\begin{assumption}[Bounded Mixed Partial Derivative]
\label{nablaFxl_bnd}
Assume that the mixed partial derivative $\nabla_{x\lambda}^2F$ is uniformly bounded in operator norm, i.e.
\[
\|\nabla_{x\lambda}^2F(x,\lambda)\|\le E
\]
for all $(x,\lambda)$ under consideration.
\end{assumption}

\subsection{\texttt{stocBio} and \texttt{stocHOAG}}
We begin by reviewing the stochastic bilevel optimisation algorithm proposed by Ji et al., which serves as the starting point for the developments in this chapter. that can be used to tackle problem (\ref{outer_hoag_neu}-\ref{inner_hoag_neu}) \cite{ji2021bilevel}. Firstly, we define some of the relevant parameters:
\begin{itemize}
...
\end{itemize}
Note that the parameters are fixed throughout the optimisation. The Neumann series and its sampling will be explained in section \ref{neumann_series}. The algorithm is described in \ref{alg:stocbio}.



\begin{algorithm}[]
\caption{\texttt{stocBio} \cite{ji2021bilevel}}
\label{alg:stocbio}
\DontPrintSemicolon
...
\end{theorem}
Of note here is that, for small $\mu$, the above is bounded as $\mathcal{O}(1/\mu^5)$. As mentioned previously, our aim is to investigate alternative parameter choices that modify the dependence of the algorithm on the strong convexity parameter $\mu$ while maintaining comparable convergence guarantees. For this purpose, we will allow the parameters to change with the outer iteration. This leads to the following algorithm, which we refer to as \texttt{stocHOAG}.
...
In practice, these quantities act as mini-batch sizes. For the numerical experiments in Section \ref{sec:stoc_hoag_num} they are chosen deterministically as functions of the outer iteration index $k$, according to the parameter schedule in Theorem \ref{hoag_thm_glcv}.
...
The key ingredient of Algorithm \ref{alg:stoc_hoag} is the stochastic Neumann series used to approximate the inverse Hessian. Before analysing convergence, we establish several properties of this approximation. As per HOAG, we can obtain the exact hypergradient of the outer problem as
...
\end{equation}
However, finding the inverse $\left[\nabla_{xx} {F}(\hat{x}(\lambda_k),\lambda_k) \right]^{-1}$ is computationally expensive, and so we consider an approximation for this term. To be more precise, consider the following:
...
Motivated by this observation, and the fact that it is not generally feasible to solve the inner problem to full precision, instead of (\ref{exact_hyper}), we use the `approximate hypergradient' in line 3 of our algorithm:
\begin{equation} \label{hat_nabla_def}
                \widehat{\nabla}\mathcal{L}(\lambda_k) = \nabla_\lambda \tilde{C}(x_k,\lambda_k,\mathcal{S}_{\nabla_\lambda C}) - \nabla_{x \lambda}^2 \tilde{F}(x_k,\lambda_k,\mathcal{S}_{\nabla_{x\lambda} F})^\top v_Q.
            \end{equation}
As you will see later, the convergence analysis of our proposed algorithm is directly related to the analytic results regarding $v_Q$. We now establish these results. We first derive an expectation bound on a $Q-$length Neumann series.
...
Having established the behaviour of the Neumann approximation, we now derive corresponding bounds for the bias and variance of the approximate hypergradient $\widehat{\nabla} \mathcal{L}$. For the bias, we provide the result of Lemma 7 in \cite{ji2021bilevel}:
...
The proofs of Propositions \ref{evq_bnd}, \ref{varvq_better} and \ref{bound_nabla_hat} are deferred to Appendix C.

\subsection{Convergence Proof}
The convergence proof requires one further ingredient: a Lipschitz continuity result for the true hypergradient. We therefore recall the following result of Ghadimi and Wang (2018)\cite{ghadimi2018approximation}:
...
\end{lemma}
Finally, we require a mild lower-boundedness assumption on the outer objective.
...
\end{assumption}
We are now in a position to establish the global convergence guarantee for Algorithm \ref{alg:stoc_hoag}.
\begin{comment}
...
\begin{theorem}[Global Convergence] \label{hoag_thm_glcv}
Suppose Assumptions \ref{conv_ass}, \ref{smo_ass}, \ref{pls_ass}, \ref{nablaFxl_bnd} and \ref{inf_ass} hold.
In Algorithm \ref{alg:stoc_hoag}, assume that the outer stepsize is chosen as $\nu_k = \frac{1}{2L_\mathcal{L}}$. Suppose $\eta$ is chosen such that $\eta \mu < 1$. Let $\zeta > 0$ be some small positive number. Choose $Q_k=\left\lceil r\log(k^{1+\zeta})\right\rceil,
r>\frac{1}{-\log(1-\eta\mu)}.$. Assume also that $\lambda_k \in \mathcal{D}$ for all $k>0$.
If we choose
\begin{equation}\label{params_conv}
    T_k = \Theta(k^{1+\zeta}), \quad 
    B_k = |\mathcal{S}_{\nabla_x C,k}| = |\mathcal{S}_{\nabla_{x\lambda} F,k}| = |\mathcal{S}_{\nabla_\lambda C,k}| = \Theta(k^{1+\zeta}),
\end{equation}
then there exists a constant $C > 0$, independent of $K$, such that
\begin{equation}
\min_{k \le K} \mathbb{E}\|\nabla \mathcal{L}(\lambda_k)\|^2 \le \frac{C \,(1 + \ln K)}{K}, \quad \text{ for all } K \ge 1.
\end{equation}
...
where $\widehat{\nabla} \mathcal{L}(\lambda_k)$ is the approximate hypergradient of $\mathcal{L}$, evaluated at $\lambda_k$, as defined in step 3 of Algorithm \ref{alg:stoc_hoag}. Taking expectation, conditioning on $\lambda_k$,
\begin{align*}
...
\end{equation}
Furthermore, from \eqref{weighted_sum}, we get that
\[
\min_{k\leq K}\mathbb E\|\nabla \mathcal L(\lambda_k)\|^2
\leq
\frac{
...
\]
By \eqref{tele_ineq}, together with the estimates above, the numerator is bounded by
\[
C_0+C_1\sum_{k=1}^{K}\frac{1}{k^{1+\zeta}},
\]
for constants \(C_0,C_1>0\) independent of \(K\). Using the integral test,
\[
\sum_{k=1}^{K}\frac{1}{k^{1+\zeta}}
\leq
1+\int_1^K x^{-(1+\zeta)}\,dx
=
1+\frac{1-K^{-\zeta}}{\zeta}.
\]
Moreover, since \(\nu_k=1/(2L_{\mathcal L})\), we have
\[
\nu_k-L_{\mathcal L}\nu_k^2=\frac{1}{4L_{\mathcal L}},
\qquad
\sum_{k=1}^{K}(\nu_k-L_{\mathcal L}\nu_k^2)
=
\mathcal O(K).
\]
Therefore, for fixed $\zeta>0$, this gives an $\mathcal O(K^{-1})$ convergence rate. Since
\[
\sum_{k=1}^K\frac1{k^{1+\zeta}}
=\mathcal O(1+\log K),
\]
the bound may also be written as
\[
\min_{k\le K}\mathbb E\|\nabla\mathcal L(\lambda_k)\|^2
=\mathcal O\!\left(\frac{1+\log K}{K}\right).
\]
\end{proof}
\section{Comparison with \texttt{stocBio}}
...
Theorem~\ref{hoag_thm_glcv} shows that the simplified parameter schedule adopted by \texttt{stocHOAG} retains the convergence guarantees established for \texttt{stocBio}. We now compare the two algorithms in terms of parameter schedules, computational complexity, and their dependence on the conditioning of the inner problem. To be precise we compare the two algorithms from a few perspectives:
\begin{itemize}
    \item parameter simplicity, namely whether the proposed schedule reduces tuning while preserving convergence guarantees;
    \item conditioning, namely how the computational complexity depends on the strong convexity parameter $\mu$.
    
    %\item Relaxed parameter choices - we want to relax the parameter choices in such a way that we still maintain similar rates of convergence, %while maintaining similar sensitivity to the inner problem condition.
    %while preserving convergence guarantees and understanding how the resulting complexity depends on the conditioning of the inner problem.
    \item Separation of the outer convergence analysis from the inner solver – We separate the convergence analysis of the outer optimisation problem from the particular method used to approximately solve the inner problem. Although the theoretical development and numerical implementation in this chapter use SGD for the inner problem, the resulting bound isolates the contribution of the inner-solver error through the term $D_{1,k}$. This makes clear where alternative inner solvers could be incorporated in future work.
\end{itemize}
\begin{comment}
    We will begin by comparing the assumptions of the two methods. Stochastic HOAG shares the assumptions of \texttt{stocBio}, with the exception of \ref{nablaFxl_bnd}, the boundedness of $\nabla^2_{x \lambda} F$. %While the two methods both assume bounds on $\|x_k - \hat{x}(\lambda_k)\|$, in our case we only require that this is bounded in expectation.
stocBio and Stochastic HOAG share mostly the same parameters, but their choices vary between the two methods. Firstly, for our bound on $\mathbb{E}\|x_k - \hat{x}(\lambda_k)\|$ to be a summable sequence, it is sufficient to choose the number of inner loop steps as $\sqrt{k} + \zeta$, for small $\zeta > 0$. In comparison, \texttt{stocBio} has a fixed number of inner-loop steps, dependent on the smoothness and convexity parameters. Regarding the Neumann series subroutine, while $Q_k$ was chosen in both cases as $\mathcal{O}(\log{k})$, the choice for the batch size of the samples of $\nabla^2_{xx} F$ was different. We allowed for a constant batch size, while in the case of \texttt{stocBio},  this was taken as $BQ(1-\eta \mu)^{j-1}$, for some large constant $B$. That is, the batch size starts large initially, but decays exponentially as we progress through the inversion subroutine. Finally, the sampling for $\nabla_x C$ was also with constant batch sizes in our case, but was done as $\mathcal{O}(\log{k})$ in the case of \texttt{stocBio}. By Theorem 3 of \texttt{stocBio}, we have the following convergence result:
\[\frac{1}{K} \sum_{k=0}^{K-1} \mathbb{E}\|\nabla\mathcal{L}(\lambda_k)\|^2 = \mathcal{O}\left( 1/K \right).\]
In theory, \texttt{stocBio} has a much faster convergence rate, but the choice of such parameters as the batches for stochastic gradient and hessians was less trivial.
\end{comment}

\subsection{Gradient Complexities and the Conditioning Problem} \label{mu_comp}
We begin by recalling the notion of an $\epsilon$-accurate stationary point.
...
Ji et al. assume the following strong convexity condition, equivalent to Assumption \ref{conv_ass} ~\cite{ji2021bilevel}.
...
In practice this lower bound is never attained, as $\lambda$ is on a bounded domain, but $\mu_0$ still may be very small. Motivated by this observation, let us recall the convergence bound, obtained in the same paper:
...
Of note is the strong dependence of the inner-problem complexity on the strong convexity parameter $\mu$. Let us now inspect how our parameter choices affect the $\mu$-dependence of the resulting complexity bounds, and whether they can reduce this sensitivity while maintaining comparable convergence guarantees. Note, that, analogous to Theorem \ref{thm3}, we can derive a similar result for our case, using the consequences of Theorem \ref{hoag_thm_glcv} above. Inspect line \ref{tele_ineq} in our convergence proof. Lower-bounding the left hand side, we obtain
...
%\end{equation}
\begin{align}
\frac{1}{K}\sum_{k=1}^K \mathbb{E}\|\nabla\mathcal{L}(\lambda_k)\|^2 &\leq  \frac{L_\mathcal{L}\left(4\mathcal{L}(\lambda_1) - 4\mathcal{L}_{inf}\right)}{K} + \notag\\
&+\frac{1}{K}\sum_{k=1}^K \left(2D_{3,k}\nu_k^2 L^2_{\mathcal{L}} + 6\nu_kL_\mathcal{L}\mathbb{E}[D_{1,k}^2] + 6\nu_k D_{2,k}^2\right), \label{tele_ineq2}
\end{align}
where,
\[D_{1,k} = \sqrt{2}\left(L + \frac{L^2}{\mu} + \frac{M\tau}{\mu} + \frac{LM\rho}{\mu^2} \right) \|\hat{x}(\lambda_k) - x_k\|, \quad D_{2,k} = \sqrt{2} \frac{LM(1-\eta \mu)^{Q_k}}{\mu},\]
...
\left(
2E^2+\frac{2L^2}{|S_{\nabla_{x\lambda}F,k}|}
\right)2\eta^2M^2 \times \notag \\
\times\left(
\frac{\kappa^2}{B_k(2-\eta\mu)^2}
+
\frac{1}{|S_{\nabla_x C,k}|(2\eta\mu-\eta^2\mu^2)}
\right) \label{varbndthm2}
\end{align}
...
As we see, while the outer gradient complexity and the Hessian-vector complexity
slightly worsen, the gradient complexity of the inner problem improves by an
order of \(\mu^3\) relative to the parametrisation of Ji et al. %Let us now provide a $\mu$-convergence bound. For this purpose, we inspect the three terms $D_{1,k}, D_{2,k}, D_{3,k}$ in (\ref{varbndthm2}). For the sake of simplicity we consider only the case $s=0$. Using Lemma \ref{sgdmu}, we get that $\mathbb{E} [D_{1,k}^2] = \mathcal{O}\left(\frac{1}{\mu^6}\sum_{k=1}^K\frac{1}{T_k}\right) \approx \mathcal{O}\left(\frac{1}{\mu^6}\ln{\frac{1}{\mu}}\right)$. Next, let us inspect $D_{2,k}^2$. Substituting the value of $Q_k$ from Theorem \ref{hoag_thm_glcv}, we get that $D_{2,k}^2 \approx \mathcal{O} \left( \frac{1}{\mu^2} \ln{\frac{1}{\mu}}\right)$. Finally, inspecting the $D_{3,k}$ term, we get that
%\[\sum^{K}_{k=1} D_{3,k} = \mathcal{O}\left( \frac{\kappa^2}{B_k (2 - \eta \mu)^2} \right) \approx \mathcal{O}\left( \frac{1}{\mu^4} \ln{\frac{1}{\mu}} \right).\]
We now turn from the oracle complexity of the individual subroutines to the
dependence of the convergence bound itself on the convexity parameter
\(\mu\). The following corollary isolates the \(\mu\)-dependence of the
right-hand side of \eqref{tele_ineq2}. In particular, it clarifies that the
Neumann-series variance term in \(D_{3,k}\) is not the dominant contribution
to the final bound; the dominant contribution comes instead from the
inner-solver error term \(D_{1,k}\).
\begin{comment}
    \begin{corollary}[\(\mu\)-dependence of the convergence bound]
Under the parameter choices of Theorem \ref{hoag_thm_glcv}, the averaged convergence bound obtained from (\ref{tele_ineq2}) satisfies
\[
\frac1K\sum_{k=1}^{K}\mathbb{E}\|\nabla \mathcal L(\lambda_k)\|^2
=
O\left(
\frac{1}{K\mu^3}
+
...
\right).
\]
In particular, the dominant dependence on \(\mu\) arises from the inner-solver error term \(D_{1,k}\), while the Neumann-series variance term in \(D_{3,k}\) contributes only \(O((K\mu^2)^{-1})\) after averaging.
\end{corollary}

\begin{proof}
    Consider the contribution of the Neumann-series variance term in \(D_{3,k}\). From (\ref{varbndthm2}), this contribution is of the form
\[
\sum_{k=1}^{K}
\frac{\kappa^2}{B_k(2-\eta\mu)^2}.
\]
Since \(\kappa=L/\mu\), \(B_k=O(k^{1+\zeta})\), and \(\eta\mu<1\), we obtain
\[
\sum_{k=1}^{K}
\frac{\kappa^2}{B_k(2-\eta\mu)^2}
=
...
\]
as \(\zeta>0\). Thus this term is not the dominant contribution in the \(\mu\)-dependence. The dominant contribution instead comes from the inner-solver error term \(D_{1,k}\). Using Lemma \ref{sgdmu} and \(T_k=O(k^{1+\zeta})\), we have
\[
\sum_{k=1}^{K}\mathbb{E}[D_{1,k}^{2}]
=
O\left(
\frac{1}{\mu^6}
\sum_{k=1}^{K}\frac{1}{k}
\right)
=
O\left(\frac{1}{\mu^6}\log K\right).
\]
Since \(K=O(\mu^{-3}\epsilon^{-1})\), this gives the leading \(\mu\)-dependence in the final complexity bound. As such, bringing $\mathbb{E} [D_{1,k}^2], D_{2,k}^2, D_{3,k}$ into (\ref{tele_ineq2}), we get that the left hand side is bounded by at most $\mathcal{O}\left( \frac{1}{\mu^3} \ln{\left( \frac{1}{\mu} \right)} \right)$. Compared to the parametrisation of Ji et al. we see an improvement at least on the order of $\mu$.
\end{proof}
...
\right).
\]
In particular, the dominant dependence on $\mu$ comes from the inner-solver error term
$D_{1,k}$, rather than from the Neumann-series variance term in $D_{3,k}$.
\end{corollary}
\begin{proof}
We inspect the three error terms $D_{1,k}$, $D_{2,k}$ and $D_{3,k}$ appearing in
\eqref{tele_ineq2}. First consider the contribution to $D_{3,k}$ arising from the Neumann-series variance term. From \eqref{varbndthm2}, this term is of the form
\[
\sum_{k=1}^{K}
\frac{\kappa^2}{B_k(2-\eta\mu)^2}.
\]
Since \(\kappa=L/\mu\), \(B_k=\Theta(k^{1+\zeta})\), and \(\eta\mu<1\), we obtain
\[
\sum_{k=1}^{K}\frac{\kappa^2}{B_k(2-\eta\mu)^2}
=
\mathcal O\left(
\frac{1}{\mu^2}\sum_{k=1}^{K}\frac{1}{k^{1+\zeta}}
\right)
=
\mathcal O\left(\frac{1}{\mu^2}\right),
\]
as \(\zeta>0\). The remaining contributions to $D_{3,k}$ have the same $\mu^{-2}$ dependence and therefore do not affect the dominant $\mu$-dependence of the convergence bound.

Hence the Neumann-series variance is not the dominant contribution to
$\mu$-dependence. 

The dominant contribution instead comes from the inner-solver error term
$D_{1,k}$. Using Lemma \ref{sgdmu} and $T_k=O(k^{1+\zeta})$, we have, using the
looser bound $k^{-(1+\zeta)}\leq k^{-1}$,
\[
\sum_{k=1}^{K}\mathbb{E}[D_{1,k}^2]
=
O\left(
...
\right)
=
O\left(\frac{\log K}{\mu^6}\right).
\]
The contribution from \(D_{2,k}\) is summable in \(k\) under the choice of \(Q_k\), and its dependence on \(\mu\) is dominated by the \(\mathcal O(\mu^{-6})\) contribution arising from \(D_{1,k}\). Substituting these estimates into \eqref{tele_ineq2}
gives
\[
\frac{1}{K}\sum_{k=1}^{K}\mathbb{E}\|\nabla \mathcal L(\lambda_k)\|^2
=
...
\right),
\]
where we also used $L_C=O(\mu^{-3})$. This proves the claim.
\end{proof}
...
\end{proof}
As such, we manage to drop a factor of $\frac{1}{\mu^2}$ from the above bound. The consequences of this change in our case and for \texttt{stocBio}, we present below.
...
\end{equation}
Higher-order implicit discretisations, such as the implicit midpoint or trapezoidal rule, could also be applied to the matrix ODE \eqref{neumann_ode}. We use implicit Euler here because it leads to the simple resolvent update \eqref{impl_neu}, which is straightforward to implement using the Woodbury identity when \(\nabla_{xx}^2 \widetilde F_t\) has low-rank structure. Specifically, suppose the stochastic Hessian admits a low-rank factorization
\[
\nabla_{xx}^{2}\tilde{F}_{t}=UV^{\top},
\]
where $U,V\in\mathbb{R}^{d\times r}$ with $r\ll d$. Applying the Woodbury matrix identity,
\[
\left(I_d+\eta UV^{\top}\right)^{-1}
=
I_d-\eta U\left(I_r+\eta V^{\top}U\right)^{-1}V^{\top},
\]
reduces the inversion of the $d\times d$ matrix
\(
I_d+\eta\nabla_{xx}^{2}\tilde{F}_{t}
\)
to the inversion of the much smaller $r\times r$ matrix
\(
I_r+\eta V^{\top}U
\).
Consequently, the implicit update can be computed efficiently whenever the Hessian is low-rank or admits an accurate low-rank approximation. 
\section{Numerics} \label{sec:stoc_hoag_num}
To illustrate the theoretical developments of the previous sections, we compare the practical performance of \texttt{stocBio} and \texttt{stocHOAG} on a synthetic bilevel least-squares problem. The aim of this experiment is not to provide a comprehensive empirical evaluation, but rather to verify that the simplified parameter schedule exhibits convergence behaviour comparable to that predicted by the theory. We generate a random Gaussian train matrix $A \in \mathbb{R}^{10 \times 20}$ and a sparse solution vector $x_0 \in \mathbb{R}^{10}$. We construct the data vector $y = Ax_0 + err$, where $err \in \mathbb{R}^{10}$ is some small Gaussian error vector. Then the problem we will be trying to tackle is
\begin{align} 
...
where $\mu_0 > 0$ is a global strong-convexity parameter, that ensures the inner problem is always at least $\mu_0$-strongly convex. Obviously, in a practical setting having this parameter is not realistic, but in our case, when the optimal value $\lambda$ is almost always close to $0$, it allows us to gauge the effect of strong convexity on convergence. We will compare \texttt{stocBio} to a few variations of \texttt{stocHOAG}. The difference between these comes down to how we tackle the inner problem and the Hessian inverse. We compare three variants of \texttt{stocHOAG} against \texttt{stocBio} in order to assess the effect of the proposed modifications to the inner solver and Hessian-inverse approximation: 
\begin{enumerate}
    \item \texttt{stocHOAG} with SGD for inner problem and $v_Q$ computed via Neumann series;
    \item \texttt{stocHOAG} with conjugate gradient descent for inner problem and $v_Q$ computed via Neumann series;
    \item \texttt{stocHOAG} with SGD for inner problem and $v_Q$ updated via ISGD.
\end{enumerate}

We run for $\mu_0 = 1$, 20000 outer iterations. %The results are presented in Figure \ref{fig:stoc_bio_hoag}. %The \texttt{stocHOAG} algorithms all feature a comparable $\mathcal{O}\left( 1/K \right)$ convergence to \texttt{stocBio} in this toy problem, suggesting that we can obtain even tighter guarantees that of Theorem \ref{hoag_thm_glcv}.
Figure \ref{fig:stoc_bio_hoag} shows that all \texttt{stocHOAG} variants exhibit a convergence trend comparable to that of \texttt{stocBio} throughout the optimisation. Although the trajectories differ during the initial transient phase, they eventually display similar asymptotic behaviour, consistent with the theoretical analysis. Furthermore, replacing the Neumann series by ISGD or the inner SGD solver by conjugate gradients does not noticeably degrade convergence on this example, supporting the flexibility of the proposed framework.
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{IMG/stochoag/a_run_of_STHOAG_grad.pdf}
    \caption{Convergence comparison of \texttt{stocBio} and \texttt{stocHOAG}}
...
\section{Conclusion}
%The \texttt{stocBio} algorithm is one of the more recent approaches to stochastic bilevel optimisation. However, as with many other algorithms, designed to tackle bilevel problems, it suffers from a high sensitivity to the strong-convexity parameter $\mu$ of the inner level problem. In this chapter we introduce a reparametrisation for the algorithm that make it more robust in this regard, providing comparable practical convergence and reducing the dependence of the inner-solver complexity on $\mu$, at the cost of slightly larger outer-gradient and Hessian-vector complexities.
This chapter investigated the sensitivity of stochastic bilevel optimisation algorithms to the conditioning of the inner optimisation problem. Starting from the \texttt{stocBio} algorithm of Ji et al., we proposed a simplified variant, termed \texttt{stocHOAG}, in which the algorithm parameters are prescribed as deterministic schedules depending only on the outer iteration. We showed that this considerably reduces the tuning burden while retaining convergence guarantees comparable to those of the original algorithm.

The proposed parameterisation also provides a clearer picture of how the computational complexity depends on the strong convexity parameter of the inner problem. In particular, the gradient complexity of the inner optimisation problem improves by an order of $\mu^3$ relative to the original parameterisation, while the outer-gradient and Hessian-vector complexities increase only modestly. Furthermore, the analysis identifies the dominant source of the conditioning dependence as the inner-solver error, rather than the stochastic Neumann-series approximation itself.

Finally, we considered an alternative Hessian-inverse approximation based on implicit stochastic gradient descent and compared several variants of the proposed framework on a synthetic bilevel optimisation problem. Although these experiments are illustrative rather than exhaustive, they demonstrate that the simplified parameter schedules exhibit practical convergence behaviour comparable to that of \texttt{stocBio}, supporting the theoretical developments presented in this chapter.

\chapter{Overall Conclusions and Future Directions}

\section{Overall Conclusions}

This thesis has investigated two closely related challenges arising in sparse optimisation and Bayesian inference. The first concerns the construction of efficient sampling algorithms for probability distributions induced by nonsmooth regularisers. The second concerns the optimisation of hyperparameters in stochastic bilevel problems, where the computational cost is often dominated by repeatedly solving an inner optimisation problem. Although these topics are distinct, they share a common objective: developing computational methods that remain both practically effective and mathematically tractable in the presence of nonsmooth optimisation problems.

The principal contribution of this thesis is the introduction and analysis of Hadamard-Langevin dynamics, a new family of stochastic processes obtained through Hadamard reparameterisations of sparse priors. Rather than approximating or smoothing the underlying nonsmooth objective, the proposed approach reformulates the sampling problem in a transformed coordinate system, permitting the application of Langevin-type dynamics while preserving the target distribution. This viewpoint provides an alternative to existing smoothing and proximal approaches and demonstrates that suitable reparameterisations can substantially simplify both algorithm design and theoretical analysis.

A significant part of the thesis is devoted to establishing rigorous mathematical guarantees for these dynamics. We derive existence and uniqueness results, establish moment bounds and ergodicity, and analyse finite-time convergence towards the invariant distribution. The analysis is then extended to practical numerical discretisations, where corresponding convergence guarantees are obtained. Particular attention is given to both cylindrical and Cartesian formulations, illustrating that the proposed framework admits multiple discretisations while retaining a common theoretical foundation.

Alongside these theoretical developments, numerical experiments demonstrate that the proposed methods achieve competitive empirical performance across a range of sparse inference problems, including regression, classification and image deconvolution. While the primary emphasis of this work is theoretical rather than computational, these experiments indicate that the proposed algorithms remain practical and perform comparably to established approaches despite avoiding explicit smoothing of the underlying objective.

The second part of the thesis concerns stochastic bilevel optimisation for hyperparameter selection. Building upon existing algorithms such as \texttt{stocBio}, this thesis proposes \texttt{StocHOAG}, a simplified stochastic variant of the HOAG algorithm. Rather than introducing an entirely new optimisation framework, this work focuses on understanding how parameter choices influence convergence guarantees and computational complexity. In particular, the proposed parameter schedule substantially simplifies the tuning procedure while preserving convergence guarantees comparable to those available for \texttt{stocBio}.

A recurring theme throughout the bilevel optimisation chapters is the influence of the conditioning of the inner optimisation problem. The analysis demonstrates that the computational complexity of several algorithmic components depends strongly on the strong convexity parameter of the inner problem. By carefully reparameterising the algorithm and separating the analysis of the outer optimisation from the particular inner solver, the proposed framework provides improved robustness with respect to parameter selection while clarifying the individual contributions of the different approximation errors to the overall convergence bound.

Taken together, these results demonstrate that suitable reformulations can provide an effective way of tackling optimisation problems that are otherwise analytically or computationally challenging. Across both parts of the thesis, the emphasis has been on preserving the original mathematical problem while developing algorithms that admit rigorous theoretical analysis and remain practical to implement.

\section{Summary of Contributions}

The principal contributions of this thesis may be summarised as follows.

\begin{itemize}
    \item The introduction of Hadamard-Langevin dynamics as a new approach for sampling from sparse Bayesian models based on Hadamard reparameterisations.

    \item A rigorous theoretical analysis of the proposed dynamics, including existence, uniqueness, ergodicity and convergence properties, together with convergence guarantees for practical numerical discretisations.

    \item The development of \texttt{StocHOAG}, a simplified stochastic bilevel optimisation algorithm that retains convergence guarantees comparable to those of \texttt{stocBio} while considerably simplifying the parameter schedule.

    \item An analysis of the influence of the conditioning of the inner optimisation problem on stochastic bilevel optimisation, together with a comparison of the conditioning dependence of \texttt{StocHOAG} and \texttt{stocBio}.
\end{itemize}

\section{Future Directions}

One of the most natural directions arising from this thesis is to combine the Bayesian inference framework developed in Part~\ref{part:Langevin} with the stochastic bilevel optimisation framework developed in Part~\ref{part:HOAG}. As demonstrated in the preceding chapters, the Hadamard reparameterisation transforms a broad class of sparse optimisation problems into equivalent smooth optimisation problems posed in an expanded parameter space. This reformulation naturally opens the door to gradient-based stochastic bilevel optimisation for automatic hyperparameter selection, allowing quantities such as regularisation parameters to be learned rather than prescribed manually.

An interesting challenge immediately arises in this setting. When the optimal regularisation parameter is small, the resulting lower-level optimisation problem becomes increasingly ill-conditioned, precisely the regime investigated in Chapter~\ref{chp: stoc_cond}. Consequently, the conditioning analysis and simplified parameter schedules developed for \texttt{StocHOAG} provide a natural starting point for developing scalable bilevel optimisation methods for Hadamard-reparameterised sparse Bayesian models.

\begin{comment}
    A particularly interesting direction suggested by this thesis is to combine the two research directions developed here. Many sparse optimisation problems are naturally formulated as $\ell_1$-regularised optimisation problems,
\[
\min_x F(x,\lambda)+\lambda\|x\|_1,
\]
where the regularisation parameter $\lambda$ must be chosen appropriately. Applying the Hadamard reparameterisation transforms this into the smooth optimisation problem
\[
\min_{u,v} F(u\odot v,\lambda)
+\frac{\lambda}{2}\|u\|^2
+\frac{\lambda}{2}\|v\|^2,
\]
making it amenable to gradient-based optimisation techniques.

This naturally suggests a smooth bilevel optimisation formulation in which the regularisation parameter $\lambda$ is selected automatically. Such problems arise, for example, in inverse problems and image reconstruction, where the quality of the reconstruction depends sensitively on the choice of the regularisation parameter.

An interesting challenge arises whenever the optimal regularisation parameter is small. In this regime, the lower-level optimisation problem becomes increasingly ill-conditioned, precisely the situation analysed in Chapter~8. Consequently, the conditioning analysis and simplified parameter schedules developed for \texttt{StocHOAG} may provide an effective framework for hyperparameter optimisation in Hadamard-reparameterised sparse optimisation problems.

Beyond this connection, several theoretical questions remain open. These include extending the Hadamard-Langevin framework beyond the classes of sparse priors considered in this thesis, weakening some of the regularity assumptions required by the convergence analysis, and establishing theoretical guarantees for the implicit stochastic gradient descent Hessian inverse proposed in Chapter~\ref{chp: stoc_cond}.

Another interesting direction concerns the Pseudo-Euler Maruyama (PEM) discretisation proposed in Chapter~\ref{chp:analysis_hada_lang}. Although introduced primarily as a heuristic motivated by the Leimkuhler–Matthews method, the numerical experiments suggest that PEM exhibits substantially improved accuracy over the standard Euler--Maruyama discretisation and appears to achieve approximately second-order convergence for stationary averages. Establishing rigorous convergence guarantees, identifying the mechanism underlying this improved behaviour, and determining whether similar ideas extend to broader classes of stochastic dynamics remain interesting open problems.
\end{comment}
Beyond this connection, several further theoretical questions remain open. These include extending the Hadamard-Langevin framework beyond the classes of sparse priors considered in this thesis, weakening some of the regularity assumptions required by the convergence analysis, establishing theoretical guarantees for the implicit stochastic gradient descent Hessian inverse proposed in Chapter~\ref{chp: stoc_cond}, and understanding the unexpectedly strong empirical performance of the Pseudo-Euler Maruyama (PEM) discretisation introduced in Chapter~\ref{chp:analysis_hada_lang}.

The latter is particularly intriguing. Although introduced primarily as a heuristic motivated by the Leimkuhler--Matthews method, PEM consistently exhibited the best empirical accuracy among the Hadamard discretisations considered in this thesis and appeared to achieve approximately second-order convergence for stationary averages. At present, however, this behaviour lacks a theoretical explanation. Establishing rigorous convergence guarantees and understanding the mechanism responsible for this improved performance remain interesting directions for future work.


\section{Final Remarks}

This thesis has demonstrated that reformulating optimisation problems can simultaneously simplify their numerical solution and facilitate rigorous mathematical analysis. Although the two research directions considered in this thesis were developed independently, they suggest a common research programme centred on the use of reparameterisation and reformulation as tools for tackling challenging optimisation problems. It is hoped that the methods and analyses presented here provide a useful foundation for future developments in sparse Bayesian computation and stochastic bilevel optimisation.
...
\begin{equation} \label{lipschitz_equiv2}
...
\end{align}
where in \eqref{lamline}, we use the fact, that $\lambda_{k}-\lambda_{k+1} = \frac{1}{L}p_k$. Finally, by Cauchy-Schwarz, we have
...
Indeed, we then have $x_i^2/\eta_i = \lambda\beta v_i^2$ and so $J = (\partial(x,\eta) / \partial(u,v))$ is a $2d \times 2d$ block-matrix, where
    $$\operatorname{det}J 
    = 
    \operatorname{det}\begin{pmatrix}
     \nabla_u x(u,v) && \nabla_v x(u,v) \\
...
\section{\texttt{stocBio}/\texttt{stocHOAG}}
...
\end{align}
where, in the last inequality, we used that $\nabla_{xx}^2F \succeq \mu I$ and $\eta < 1/L$, implying $\|I-\eta\nabla_{xx}^2F\|\le 1-\eta\mu$.
\end{proof}
...
\begin{align*}
    ...
    A &= \mathbb{E} \left[ A_j \right] = I- \eta \nabla_{xx}^2 F(x_k,\lambda_k); \\
    B &= \mathbb{E} \left[\tilde{B} \right] = \nabla_x C (x_k,\lambda_k);
\end{align*}
Then 
...
%\[
%\begin{aligned}
%\operatorname{Var}(X^\top v_Q)
%&\leq
%2\operatorname{Var}(X)\mathbb{E}\|v_Q\|^2
%+
%2\|\mathbb{E}[X]\|^2\operatorname{Var}(v_Q).
%\]
...
\frac{M^2}{\mu^2},
\]
where we used Proposition~\ref{evq_bnd}. Therefore,
\[
\begin{aligned}
\operatorname{Var}(X^\top v_Q)
&\leq
...
Substituting the bound from Proposition~\ref{varvq_better} yields
\[
\begin{aligned}
\operatorname{Var}(\widehat{\nabla}\mathcal{L}(\lambda_k))
\leq\;&
