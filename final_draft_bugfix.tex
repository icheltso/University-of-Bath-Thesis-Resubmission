...
\end{equation}
where, assuming $\nabla_{xx}^2 \tilde{F}_t$ is low-rank, we can leverage the Woodbury matrix identity to invert the system. Specifically, suppose the stochastic Hessian admits a low-rank factorization
\[
...
I_r+\eta V^{\top}U
\).
Consequently, the implicit update can be computed efficiently whenever the Hessian is low-rank or admits an accurate low-rank approximation. The update (\ref{impl_neu}) is a step of implicit stochastic gradient descent. 
