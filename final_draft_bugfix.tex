...
\end{equation}
where $f$ is an adapted process with respect to the filtration $\{\mathcal{F}_t\}$, generated by the Brownian motion $W_t$. Assuming $f$ is square-integrable, the It\^{o} Integral satisfies the It\^{o} isometry \cite{pavliotis_book}:
...
\end{equation}
where $\mathcal{F}_s$ denotes the filtration generated by $W(s)$. In stochastic calculus differentiation is redefined via It\^{o}'s lemma:
