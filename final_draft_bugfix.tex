...
\begin{definition}[Positive definite]
    \item[$A \succ 0$] Matrix positive definiteness. For a symmetric matrix
...
\begin{figure}[htp]
    ...
    \caption{Image deconvolution by regularization; Reference original and blurred images found in Figure \ref{fig:blurry_tiger}. Left: Optimal $\lambda$ w.r.t. SNR;  Right: Regularization path: SNR vs $\lambda$.}\label{fig:deblurry_tiger}
\end{figure}
...
x_t=(y_t,z_t,v_t)
\]
be a solution of the Cartesian system~(\ref{cyl}), and define
\[
...
(u_t,v_t)
\]
satisfies the Hadamard--Langevin system~(\ref{uvproc}).

Moreover, if the initial distribution of \(x_0\) is cylindrical, then the law of
\((u_t,v_t)\) is the cylindrical marginal of the law of \(x_t\).
\end{theorem}
...
follows directly from~(\ref{cyl}). Hence \((u_t,v_t)\) satisfies
\((\ref{uvproc})\).
...
u_t=(y_t\odot y_t+z_t\odot z_t)^{1/2}
\]
induces a solution \((u_t,v_t)\) of~(\ref{uvproc}).

Since \(x_t\) is well posed, strong Feller, irreducible, and satisfies
the moment bounds established above, it follows that \((u_t,v_t)\) is
...
\]
If \(A=0\), there is nothing to prove. Otherwise, dividing by \(A\) yields
\[
A
\leq
c_2
