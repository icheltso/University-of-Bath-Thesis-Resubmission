...
d{\|v_t\|^2} & = \bp{-2\,\lambda\, {\|v_t\|^2} - 2{(u_t\odot v_t)^\top\nabla G(u \odot v)\strutb} + \frac{2\,d}{\beta}}\,dt+\sqrt{\frac{2}{\beta}}2v_t^\top dW_t^2.\label{Ito_norm_2_v}
\end{align}
...
2. For each $T>0$ and $\epsilon>0$, we have
 $\mean{\pp{\int_0^T \frac{1}{u_{i,s}^{2-\epsilon}}\,ds}^q} <\infty $ for all $1 \le q < 1$.
...
Despite this limitation, numerical evidence suggests that the discretized invariant measure $\pi_{\tstep}$ should converge to the true invariant measure $\pi$ as $\Delta t \to 0$. See Subsection \ref{subsec: 1d_experiment}, for example.
\end{remark}
