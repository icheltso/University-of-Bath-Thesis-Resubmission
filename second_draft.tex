...
\begin{abstract}
    Sparse optimization is playing an increasingly prominent role in the modern world, in such fields as compressed sensing, inverse problems and machine learning. A way to achieve sparse solutions is to leverage sparsity-enforcing regularizers, such as the $l_1$-norm. However, the study of regularization parameters is a relatively new topic. The choice of parameters is often nontrivial as it is unclear what metric one should use to do this. This is an important practical issue, as choosing the right parameter can visibly influence the quality of a solution.  In the first part of this thesis, we analyze the various methods to tackle sparse-regularized problems as well as the practical setting in which they are used, with emphasis on numerics. In the second part we explore stochastic bilevel optimization techniques for parameter selection in sparse-regularized problems, with an emphasis on the conditioning of the inner problem. The overarching, unifying theme of this thesis will be the use of Hadamard reparameterizations for sparse regularizers.
...
In machine learning, regularization typically augments the minimization of a loss function with a penalty term. Suppose we aim to learn a predictor $x(u)$ from samples $u_i$ and labels $y_i$, minimizing a loss $L$. Then regularization takes the form
\begin{equation} \label{reg_ml}
    x^* = \argmin_x \sum_{i=1}^n L(x(u_i), y_i) + \lambda R(x),
\end{equation}
...
We define the \textit{proximal operator}. Let $\phi : \mathbb{R}^n \xrightarrow[]{} \mathbb{R} \cup \{+
\infty\}$. Then $\mathrm{prox}_\phi: \mathbb{R}^n \xrightarrow[]{} \mathbb{R}^n$ is defined as
...
explicitly. This is
\[x = x_k - \frac{1}{\gamma} \nabla f(x_k),\]
i.e. the gradient descent step. Hence it is natural to consider
\begin{align*}
...
\begin{itemize}
    ...
    \[
        \liminf_{y \to x} g(y) \;\geq\; g(x), \quad \forall x \in \mathbb{R}^n,
    \]
    ...
    \[
        ...
        \quad \forall x,y \in \mathbb{R}^n,\; \theta \in [0,1].
    \]
\end{itemize}
\end{definition}
...
\end{proof}
From the above, we see that $z \in (I + \gamma \partial g)^{-1}(x)$ if and only if $z = \mathrm{prox}_{\gamma g}(x).$ Furthermore, this tells us that the point-to-set operator $(I + \gamma \partial g)^{-1}$ is in fact a function.
We now provide the most important result of this section.
...
The ISTA algorithm is stable and offers a cheap per-iteration cost, for regularizers such as $\ell_1$. However it also has a relatively slow $\mathcal{O}(1/k)$ convergence rate. As such, related algorithm, FISTA (or Fast ISTA), was developed, where the proximal (or shrinkage) operator is applied not to the current iterate $x_k$, but to a linear combination of the current and previous iterates, $x_k$ and $x_{k-1}$. This algorithm was shown to have an improved $\mathcal{O}(1/k^2)$ convergence rate \cite{beck2009fista}.
...
As was shown by Poon\cite{poon}, (\ref{xlasso}) can be reformulated into a smooth problem. To be more precise, we present the important results of \cite{poon}:
...
For a set of hyper-parameters $\lambda \in \mathcal{D} \subset \mathbb{R}^r$, consider the optimization problem:
\begin{equation} \label{regular}
    \min _{x \in \mathbb{R}^n} F(x,\lambda) \triangleq f(x) + g(x,\lambda),
\end{equation}
where $f: \mathbb{R}^n \xrightarrow[]{} \mathbb{R}$ is some smooth function. Both for the case, when $g: \mathbb{R}^n \times \mathcal{D} \xrightarrow[]{} \mathbb{R}$ is smooth and non-smooth, various algorithms have been developed to tackle (\ref{regular}). However, the choice of the hyperparameter $\lambda$ remains an open question. One approach, that we will be exploring below, is that of \textit{hyperparameter optimization}. We are interested in solving bilevel problems of the form

\begin{align} 
%&\min _{\lambda \in \mathcal{D} \subset \mathbb{R}^r} L\left(\lambda \right) \triangleq C(\hat{x}(\lambda), \lambda) \label{bilev1}\\
&\min _{\lambda \in \mathcal{D} \subset \mathbb{R}^r} C(\hat{x}(\lambda), \lambda) \label{bilev1}\\
&\text { s.t. } \hat{x}(\lambda) =\underset{x \in \mathbb{R}^{n}} \argmin {F(x, \lambda)} \label{inprob},
\end{align}
where $C$ is some criterion for the optimal $\lambda$ for the inner problem solution. This could be something like cross-validation loss, i.e. if we split a sample matrix $A$ into $A_{train}$ and $A_{test}$, then
\[C(\hat{x}(\lambda),\lambda) = \|A_{test} \hat{x}(\lambda) - b_{test}\|_2^2.\]
% \begin{align} 
%&\min _{\lambda \in \mathbb{R}^r} L\left(x ; y^{*}(x)\right) \label{bilev1}\\
%&\text { s.t. } y^{*}(x)=\underset{y \in \mathbb{R}^{m}} \argmin {g(x, y)} \label{inprob}
%\end{align}
One way to look at this is to consider the above as a single-level problem with constraints the optimality conditions of the inner problem. However, depending on the inner problem, the number of constraints may be large and the feasible set may be non-convex. %Cite here
...
Here we have to solve the inner problem to a high enough accuracy, so that the gradient approximation $p_k$ is close enough to $\nabla L$. Otherwise we might have divergence in the outer problem. Hence we have the complexity-accuracy trade-off between solving the inner problem and solving the outer problem. To be more precise, we present Pedregosa's main results about the convergence of HOAG. The proofs for these can be found in the appendix.
...
So far we worked with the assumption that the inner problem (\ref{inprob}) is smooth. %For example, when we use ridge regression. 
We now consider when this is not the case. To be specific, consider the bi-level problem:
\begin{align} 
...
\end{align}
where $g(x,\lambda) = \sum_{j=1}^\infty g_j(x,\lambda)$ is not differentiable. We can no longer use the same approach as in Section \ref{section-hoag}, since the inner problem is not smooth. We'll consider two approaches to tackling this problem. The first stems from the paper by Bertrand et al. \cite{bertrand2022implicit} and tackles the nonsmooth problem head-on, while the second, by Poon and Peyr\'{e} \cite{poon}, reparametrizes (\ref{inprob_ns}) into a smooth problem, wherein we can use the same techniques as before. We begin with the former.

\subsection{Notation}
In the latter part of this report, we utilize indexing notations, which we elaborate as follows. We denote the restriction of a vector $x \in \mathbb{R}^n$ to a set $S \subseteq [n]$, as
\[x_S = \{x_i | i \in S\}.\]
For a differentiable function $G:\mathbb{R}^n \xrightarrow[]{} \mathbb{R}$, we denote the restriction of the gradient of $G$ to set $S \subseteq n$ as $\nabla_{S} G$, where
\[\left(\nabla^2_{S} G \right)_{i} = \left\{\frac{\partial G}{\partial x_i} \bigg| i \in S \right\}.\]
Similarly, for a twice differentiable function $F:\mathbb{R}^n \xrightarrow[]{} \mathbb{R}$, we denote the restriction of the Hessian of $F$ to sets $S,T \subseteq n$ as $\nabla^2_{S,T} F$, where
\[\left(\nabla^2_{S,T} F \right)_{ij} = \left\{\frac{\partial F}{\partial x_i \partial x_j} \bigg| i \in S, j \in T \right\}.\]

%\subsection{Assumptions and Preliminaries}
\subsection{Preliminaries}
The core of this approach stems from the fact, that, for some non-smooth regularizers $g$, although the inner problem is not smooth, the proximal operator, introduced previously,
\begin{equation*}
    prox_{\lambda g}(x) =\underset{z} \argmin \frac{1}{2}\|z-x\|_2^2 + \lambda g(z)
\end{equation*}
is differentiable at the optimum. The assumptions made on $g$, that ensure this, will be introduced later on. We can then leverage the associated fixed-point equation
\begin{equation*}
    \hat{x}(\lambda) = prox_{\lambda g} \left(\hat{x}(\lambda) - \gamma \nabla f(\hat{x}(\lambda))\right),
\end{equation*}
to derive an equation for the Jacobian of $\hat{x}(\lambda)$. For this, we must first ensure that the regularization path $\lambda \mapsto \hat{x}(\lambda)$ is almost everywhere differentiable, such as in the case of LASSO, as was shown by Mairal \& Yu \cite{mairal2012complexity} (see Figure \ref{fig:reg_path}). %As such, we can consider the derivative $\frac{\partial \hat{x}(\lambda)}{\partial \lambda}.$ 
%The core of the paper by Bertrand et al. \cite{activeset} relies on the fact that the fixed-point equation of proximal gradient descent is differentiable over the active set of the PGD solution. 
For a generic regularizer, the differentiability of the regularization path is an open question.
\begin{figure}[h!]
    \centering
    \includegraphics[width = 0.6\textwidth]{IMG/prox_reg_path.pdf}
    \caption{Regularization path for $\hat{x}(\lambda) = \argmin_x \|Ax-y\|_2^2 + \lambda \|x\|_1$, for randomly generated $A \in \mathbb{R}^{100 \times 200},y \in \mathbb{R}^{100}$.}
    \label{fig:reg_path}
\end{figure}
%This section will make use of the proximal operator, defined previously:
%\begin{equation*}
%    prox_{g}(x) =\underset{z} \argmin g(z) + \frac{1}{2}\|z-x\|_2^2
%\end{equation*}
%As before, we assume that $f$ in $\ref{regular}$ is a convex, differentiable function, with L-Lipschitz gradient.
%We will use a set of assumptions:
%\begin{itemize}
%    \item We assume that $f$ in $\ref{regular}$ is a convex, differentiable function, with L-Lipschitz gradient.
%\end{itemize}

\subsection{Direct Approach}
\label{section: bertrand}
Because of the nonsmooth inner problem, (\ref{opt_cond_smooth}) and (\ref{ift_opt}) no longer hold and we cannot proceed with chain rule as in the previous section. Throughout this section we will present both general results and their application to LASSO (as in \ref{inprob_ns}), as we are interested in sparse regularization in particular. Consider again, the inner problem
\begin{equation*} \label{regular_sep}
    \min _{x \in \mathbb{R}^n} F(x,\lambda) \triangleq f(x) + \sum_{j=1}^\infty g_j(x,\lambda),
\end{equation*}
with a separable regularizer. We make the following assumptions:
\begin{assumption}[Smoothness]
\label{smooth_a}
    We assume that $f$ is a convex, differentiable function, with L-Lipschitz gradient.
\end{assumption}
\begin{assumption}[Proper,lower semicontinuous,convex]
\label{pcc_a}
    For all $\lambda \in \mathbb{R}^r$, for any $j \in [p]$, $g_j(\cdot,\lambda): \mathbb{R} \xrightarrow[]{} \mathbb{R}$ is proper, lower semicontinuous and convex.
\end{assumption}
\begin{assumption}[Non-degeneracy]
\label{nondeg_a}
The problem admits a least one solution:
    \begin{equation*}
        \underset{x \in \mathbb{R}^p} \argmin F(x,\lambda) \neq \emptyset
    \end{equation*}
    and, for any $\hat{x}$ solution of $\underset{x \in \mathbb{R}^p} \argmin F(x,\lambda)$, we have
    \[-\nabla f(\hat{x}) \in ri\left(\partial_x g(\hat{x},\lambda)\right).\]
\end{assumption}
We introduce a new definition.
\begin{definition}[Generalized Support]
For a solution $\hat{x} \in \argmin_{x \in \mathbb{R}^p} F(x,\lambda)$, its generalized support $\hat{S} \subseteq [p]$ is the set of indices $j \in [p]$ such that $g_j$ is differentiable at $\hat{x}_j$:
    \[\hat{S} \triangleq \{j \in [p]: \partial_x g_j(\hat{x}_j) \text{ is a singleton}\}.\]
\end{definition}
For example, the generalized support of the 1-norm $\|\hat{x}\|_1$ are the indices, where its subgradient takes on a single value. That is
\[\hat{S} = \{j \in [p]: \hat{x}_j \neq 0\}.\]
Note, that for $\hat{x}_j \neq 0$, the subgradient takes on values of either -1 or 1, depending on the index sign. An important feature of proximal gradient descent is its finite support identification property. That is, if assumptions (\ref{smooth_a})-(\ref{nondeg_a}) hold, there exists $K \geq 0$, such that for all $k \geq K$ and $j \notin \hat{S}$, $x_j = \hat{x}_j$ \cite{liang2017activity}. As you will see in lemma \ref{diffprox_l} below, this feature is critical to the algorithm, as it ensures differentiability of the prox.%This feature will allow us to control the precision to which we solve the inner problem in Algorithm \ref{alg:active_set}.
\begin{assumption}[Locally $\mathcal{C}^2$ and $\mathcal{C}^3$]
\label{locsmooth_a}
The map $x \xrightarrow[]{} f(x)$ is locally $\mathcal{C}^3$ around $\hat{x}$. For all $\lambda \in \mathbb{R}^r$, for all $j \in \hat{S}$ the map $g_j(\cdot,\lambda)$ is locally $\mathcal{C}^2$ around $\hat{x}_j$.
\end{assumption}
Assumption \ref{locsmooth_a} holds for the $l_1$-norm: the support indices are one that are non-zero, and so $g_j(x,\lambda) = \lambda|x_j| \xrightarrow[]{\partial} \lambda\mathrm{sign}(x_j) \xrightarrow[]{\partial} 0.$ Note that $\mathrm{sign}(x_j)$ is unique for active indices $j$.
\begin{assumption}[Restricted Injectivity]
\label{resinj_a}
Let $\hat{x}$ be a solution of $\underset{x \in \mathbb{R}^p} \argmin F(x,\lambda)$, with generalized support $\hat{S}$. Then the solution $\hat{x}$ satisfies 
    \[\nabla^2_{\hat{S},\hat{S}}f(\hat{x}) \succ 0,\]
\end{assumption}
where $\nabla^2_{\hat{S},\hat{S}}f(\hat{x})$ denotes the Hessian, with entries restricted to the generalized support. For example, for LASSO, the solution satisfies $A_{\hat{S}}^TA_{\hat{S}} \succ 0,$ which holds if the entries of $A$ are drawn from a continuous probability distribution\cite{tibshirani2013lasso}.

\begin{lemma}[Liang et al. 2017, Prop 4.1 \cite{liang2017activity}]
\label{lianglem}
Assume that there exists a neighbourhood $\Lambda$ of $\lambda$ such that Assumptions \ref{smooth_a}, \ref{nondeg_a} and \ref{resinj_a} are satisfied for every $\lambda \in \Lambda$. Then for every $\lambda \in \Lambda$, 
\begin{equation} \label{inner_prob}
    %\hat{x}(\lambda) \in \underset{x \in \mathbb{R}^p} \argmin F(x,\lambda) \triangleq f(x) + \underbrace{\sum_{j=1}^p g_j(x_j,\lambda)}_{\triangleq g(x,\lambda)}
    \hat{x}(\lambda) \in \underset{x \in \mathbb{R}^p} \argmin \left\{ f(x) + \sum_{j=1}^p g_j(x_j,\lambda) \right\}
\end{equation}
has a unique solution, and the map $\lambda \mapsto \hat{x}(\lambda)$ is well-defined on $\Lambda.$
\end{lemma}
This guarantees to us that we can find a unique solution for the inner problem, given any value of $\lambda$. We will not provide a proof of this lemma, but note, that, in the case of LASSO, we have a unique solution given by the subgradient optimality condition. Given the problem $\min_x \|Ax-b\|_2^2 + \lambda\|x\|_1$, assuming $A_{\hat{S}} = [A_s]_{s \in \hat{S}}$ has full rank, the solution is unique and we have
\[\hat{x}_{\hat{S}}(\lambda) = (A_{\hat{S}}^\top A_{\hat{S}})^{-1}(A^\top_{\hat{S}}b - \lambda \eta), \quad \hat{x}_{\hat{S}^c}(\lambda) = 0,\]
where $\eta = \mathrm{sign}(A^\top(Ax^*-b)).$
From here we considered the fixed-point equation
\begin{equation*}
    \hat{x}(\lambda) = prox_{\lambda g} \left(\hat{x}(\lambda) - \gamma \nabla f(\hat{x}(\lambda))\right),
\end{equation*}
corresponding to proximal gradient descent reaching optimality. As differentiability of the $\mathrm{prox}$ with respect to $\hat{x}$ is one of the main challenges to this approach, we present the following result with proofs, by Klopfenstein et al. \cite{klopfenstein2020model}, which is vital for Theorem \ref{nonsmooth_gradient}.

%\begin{figure}
%    \centering
%    \includegraphics{reg_path_l1.png}
%    \caption{Caption}
%    \label{fig:my_label}
%\end{figure}

\begin{lemma}[Klopfenstein et al. 2020 \cite{klopfenstein2020model}, Lemmas 2 and 3]
\label{diffprox_l}
Let $0 < \gamma_j \leq 1/L_j.$ Let $\lambda \in \mathbb{R}^r$ and $\Lambda$ a neighbourhood of $\lambda$. Consider a solution $\hat{x}$ of (\ref{inner_prob}) and $\hat{S}$ its generalized support. Suppose
\begin{enumerate}
    \item Assumptions \ref{smooth_a}, \ref{pcc_a} and \ref{locsmooth_a} hold
    \item Assumption \ref{nondeg_a} hold on $\Lambda$.
\end{enumerate}
Then the map $x \mapsto prox_{\gamma_j g_j}(x_j - \gamma_j \nabla_j f(x))$ is differentiable at $\hat{x}$. Moreover,
\begin{enumerate}
    \item $\forall j \in \hat{S}$, the map $x \mapsto prox_{\gamma_j g_j}(x_j - \gamma_j \nabla_j f(x))$ is differentiable at $\hat{x}_{\hat{S}}$;
    \item $\forall j \in \hat{S}^c $, the map $x \mapsto prox_{\gamma_j g_j}(x_j - \gamma_j \nabla_j f(x))$ is differentiable at $\hat{x}_{\hat{S}^c}$ with gradient 0.
\end{enumerate}

\end{lemma}

\begin{proof}
We will prove for each case component-wise. \\
\textbf{(i)} We begin with the case $j \in S.$
By assumption \ref{locsmooth_a}, we know there exists a neighbourhood $U$ of $\hat{x}_j$, such that $g_j(\cdot, \lambda)$, restricted on $U$, is locally twice continuously differentiable on $U$. In particular, it is differentiable at $\hat{x}_j$. Let $(u,v) \in U \times \mathbb{R}^n$, where we have chosen $v = (\mathrm{Id} + \gamma g'_j)(u).$ Then, using Lemma \ref{prox_subdif}, we get
\begin{equation} \label{uv_prox}
    u = prox_{\gamma g_j}(v) \in U.
\end{equation}
Now, let $H(u) = u + \gamma\partial g_j(u).$ By assumption, $g$ is twice differentiable at $u$, so $H$ is continuously differentiable and
\[H'(u) = 1 + \gamma g''_j(u).\]
Define $F(u,v) \triangleq v - H(u).$ Then $F$ is continuously differentiable and $F(v,u) = 0$. By assumption, $g_j$ is convex, so $g''_j \geq 0$. Thus, $F(u,v)$ is invertible, since 
\[\partial_u F(u,v) = - H'(u) = -1 - \gamma g''_j(u) < -1.\]
Now fix $a =  \hat{x}_j - \gamma \nabla f_j(\hat{x}), \quad b =\hat{x}_j.$ By the Implicit Function Theorem, there is an open neighourhood $V \in \mathbb{R}$ of $a$, such that there exists a unique continuously differentiable function $h:V \xrightarrow[]{} \mathbb{R},$ such that $h(a) = b.$ But by (\ref{uv_prox}), $h = prox_{\gamma g_j}$ on $V$ and is differentiable at $a = \hat{x}_j - \gamma \nabla f_j(\hat{x}).$ Thus, $x \mapsto prox_{\gamma g_j}(x_j)$ is differentiable at $\hat{x}_j - \gamma \nabla f_j(\hat{x}_j)$ and so $x \mapsto prox_{\gamma g_j}(x_j - \gamma \nabla f_j(x_j))$ is differentiable at $\hat{x}_j$.\\
\textbf{(ii)}
The subgradient $\partial g_j$ is a closed, convex set. So let, without loss of generality, $\partial g_j = [a,b]$ for some $a,b \in \mathbb{R}$. Let $\hat{z}_j = \hat{x}_j - \gamma \nabla f_j(\hat{x})$. By assumption $\ref{nondeg_a}$, we also have
\[-\nabla f(\hat{x}) \in ri(\partial g_j(\hat{x},\lambda)) = (a,b).\]
By the optimality condition of the fixed-point equation, we then have
\begin{align*}
    \frac{1}{\gamma}(\hat{z}_j - \hat{x}_j) \in (a,b) &\iff \\
    \hat{z}_j \in (\hat{x}_j+ \gamma a,\hat{x}_j+ \gamma b).
\end{align*}
Note that 
$$v \in (\hat{x}_j+ \gamma a,\hat{x}_j+ \gamma b) \iff \frac{1}{\gamma}(v - \hat{x}_j) \in ri(\partial g_j(\hat{x},\lambda) \implies \mathrm{prox}_{\gamma g_j}(v) = \hat{x}_j.$$
Setting $v = \hat{x}_j - \gamma \nabla f_j(\hat{x})$, we see that $x \mapsto prox_{\gamma_j g_j}(x_j - \gamma_j \nabla_j f(x))$ is a constant function around $\hat{x}_j.$ Since $f$ is twice continuously differentiable at $\hat{x}_j$, $x \mapsto prox_{\gamma_j g_j}(x_j - \gamma_j \nabla_j f(x))$ is differentiable with gradient 0.

\end{proof}

We can now tackle the implicit differentiation problem for a non-smooth regularizer.

\begin{theorem}[Non-smooth implicit formula] 
\label{nonsmooth_gradient}
Suppose Assumptions \ref{smooth_a}, \ref{pcc_a}, and \ref{locsmooth_a} hold. Let $0 < \gamma \leq 1/L$, where $L$ is the Lipschitz constant of $\nabla f$. Let $\lambda \in \mathbb{R}^r$, $\Lambda$ be a neighbourhood of $\lambda$, and $\Gamma^\Lambda \triangleq \left\{\hat{x}(\lambda) - \gamma \nabla f(\hat{x}(\lambda)) : \lambda \in \Lambda \right\}$. In addition,
\begin{itemize}
    \item (H1) Suppose Assumptions \ref{nondeg_a} and \ref{resinj_a} hold on $\Lambda$.
    \item (H2) Suppose $\lambda \mapsto \hat{x}(\lambda)$ is continuously differentiable on $\Lambda$ (see Fig. \ref{fig:reg_path}).
    \item (H3) Suppose for all $z \in \Gamma^\Lambda$, $\lambda \mapsto prox_{\gamma g(\cdot,\lambda)}(z)$ is continuously differentiable on $\Lambda$.
    \item (H4) Suppose $\partial_z prox_{\gamma g(\cdot,\lambda)} $ and $\partial_\lambda prox_{\gamma g(\cdot,\lambda)}$ are Lipschitz continuous on $\Gamma^\Lambda \times \Lambda$.
\end{itemize}

Let $\hat{x} \triangleq \hat{x}(\lambda)$ be the solution of (\ref{inner_prob}), $\hat{S}$ its generalized support of cardinality $\hat{s}$. Then the Jacobian $\hat{\mathcal{J}}$ of $\hat{x}(\lambda)$ is given by the following formula:
\[\hat{z} = \hat{x} - \gamma \nabla f(\hat{x}), \text{ and } A \triangleq \mathrm{Id}_{\hat{s}} - \partial_z prox_{\gamma g(\cdot,\lambda)}(\hat{z})_{\hat{S}} \odot \left( \mathrm{Id}_{\hat{s}} - \gamma \nabla^2_{\hat{S},\hat{S}} f(\hat{x})  \right):\]
\begin{align}
    \hat{\mathcal{J}}_{\hat{S}^c:} &= \partial_\lambda prox_{\gamma g(\cdot,\lambda)}(\hat{z})_{\hat{S}^c}, \label{J_inact}\\
    \hat{\mathcal{J}}_{\hat{S}:} &= A^{-1} \left( \partial_\lambda prox_{\gamma g(\cdot,\lambda)}(\hat{z})_{\hat{S}} - \gamma \partial_z prox_{\gamma g(\cdot,\lambda)}(\hat{z})_{\hat{S}} \odot  \nabla^2_{\hat{S},\hat{S}^c} f(\hat{x}) \hat{\mathcal{J}}_{\hat{S}^c:}  \right). \label{J_act}
\end{align}
\end{theorem}

\begin{proof}
By Lemma \ref{lianglem} and the necessary assumptions, (\ref{inner_prob}) has a unique minimizer and $\lambda \mapsto \hat{x}(\lambda)$ is well-defined on $\Lambda$. Consider the proximal gradient method fixed point equation:
\begin{equation} \label{fpe}
    \hat{x}(\lambda) = prox_{\lambda g} \left(\hat{x}(\lambda) - \gamma \nabla f(\hat{x}(\lambda))\right).
\end{equation}

By Lemma \ref{lianglem} and assumptions \ref{smooth_a}, \ref{locsmooth_a} and (H2),(H3),(H4), we have that $\lambda \mapsto \psi \left( \hat{x}(\lambda) - \gamma \nabla f(\hat{x}(\lambda), \lambda\right) \triangleq prox_{\lambda g} \left(\hat{x}(\lambda) - \gamma \nabla f(\hat{x}(\lambda))\right)$ is differentiable at $\lambda$. Differentiating w.r.t. $\lambda$, we thus get
\[\frac{\partial\hat{x}(\lambda)}{\partial\lambda} = \hat{\mathcal{J}} = \frac{\partial \psi}{\partial \hat{z}} \frac{\partial \hat{z}}{\partial \hat{x}(\lambda)} \frac{\partial \hat{x}(\lambda)}{\partial \lambda} + \frac{\partial \psi}{\partial  \lambda}, \text{ or }\]
\begin{equation} \label{fpe_diff}
    \hat{\mathcal{J}} = \partial_z prox_{\gamma g(\cdot,\lambda)}(\hat{z}) \odot \left( \mathrm{Id} - \gamma \nabla^2 f(\hat{x})\right) \hat{\mathcal{J}} + \partial_\lambda prox_{\gamma g(\cdot,\lambda)}(\hat{z}),
\end{equation}
with $\hat{z} = \hat{x} - \gamma \nabla f(\hat{x})$. In addition to $0 < \gamma < 1/L \leq 1/L_j$, the separability of $g$ and assumptions \ref{smooth_a},  \ref{nondeg_a} and \ref{locsmooth_a} ensure through lemma \ref{diffprox_l} that for any $j \in \hat{S}^c$,
\begin{equation} \label{inact_prox}
    \partial_z prox_{\gamma g(\cdot,\lambda)}(\hat{x}_j - \gamma \nabla_j f(\hat{x})) = 0.
\end{equation}
From this we get equation (\ref{J_inact}), i.e., that for all $j \in \hat{S}^c$:
\begin{equation} \label{inact_grad}
    \hat{\mathcal{J}_j} = \partial_\lambda prox_{\gamma g(\cdot,\lambda)}(\hat{x}_j - \gamma \nabla_j f(\hat{x}))
\end{equation}
From (\ref{inact_prox}) and (\ref{inact_grad}), we get that, restricted to the generalized support, the Jacobian satisfies
\begin{equation} \label{act_grad}
    \left( \mathrm{Id}_{\hat{s}} - \partial_z prox_{\gamma g(\cdot,\lambda)}(\hat{z})_{\hat{S}} \odot (\mathrm{Id}_{\hat{s}} - \gamma \nabla^2_{\hat{S},\hat{S}} f(\hat{x}))\right) \hat{\mathcal{J}}_{\hat{S:}} = 
\end{equation}
\[-\gamma \partial_z prox_{\gamma g(\cdot,\lambda)}(\hat{z})_{\hat{S}} \odot \nabla^2_{\hat{S},\hat{S}^c} f(\hat{x}) \hat{\mathcal{J}}_{\hat{S}^c:} + \partial_\lambda prox_{\gamma g(\cdot,\lambda)}(\hat{z})_{\hat{S}:}.\]
By Cauchy-Schwartz, and the fact, that $0 < \gamma \leq 1/L$, we get the following inequality:
\[\|\partial_z prox_{\gamma g(\cdot,\lambda)}(\hat{z})_{\hat{S}} \odot (\mathrm{Id}_{\hat{s}} - \gamma \nabla^2_{\hat{S},\hat{S}} f(\hat{x}))\|_2 \leq \|\partial_z prox_{\gamma g(\cdot,\lambda)}(\hat{z})_{\hat{S}}\|_2 \cdot \|\mathrm{Id}_{\hat{s}} - \gamma \nabla^2_{\hat{S},\hat{S}} f(\hat{x})\|_2 < 1.\]
Now, $(I-A)$ is singular $\iff$ $1$ is an eigenvalue of $A$. This cannot be the case if $\|A\|_2 = \sup_{x \in \mathbb{R}^n} \frac{\|Ax\|_2}{\|x\|_2} < 1$. Hence,
\[A \triangleq \mathrm{Id}_{\hat{s}} - \partial_z prox_{\gamma g(\cdot,\lambda)}(\hat{z})_{\hat{S}} \odot (\mathrm{Id}_{\hat{s}} - \gamma \nabla^2_{\hat{S},\hat{S}} f(\hat{x}))\]
is invertible and we get the equation (\ref{J_act}).
\end{proof}
In our specific case, with $g(x,\lambda) = e^\lambda\|x\|_1$, both the proximal operator, as well as its partial derivatives exist in closed form.
Denote the Soft-Thresholding operator as
\[ST(x_i,\lambda) = \mathrm{sign}(x_i)\max{((|x_i|-\lambda),0)}.\]
We then have
\[prox_{\gamma g(\cdot,\lambda)}(z_j) = ST(z_j,e^\lambda),\]
\[\partial_{z} prox_{\gamma g(\cdot,\lambda)}(z_j) = |\mathrm{sign}(ST(z_j,e^\lambda))|,\]
\[\partial_\lambda prox_{\gamma g(\cdot,\lambda)}(z_j) = -e^\lambda\mathrm{sign}(ST(z_j,e^\lambda)).\]
Using this and the result of Theorem \ref{nonsmooth_gradient}, we can implement the algorithm.

\subsection{Algorithms}
We now state the algorithms that arise as a consequence of Theorem \ref{nonsmooth_gradient}.

\begin{algorithm}[H] 
  \KwData{Input $\lambda \in \mathbb{R}^r, \epsilon > 0, \gamma >0$}
 %\KwResult{Updated weights $w^{[l]}_{jk}$ and biases $b^{[l]}_j$}
    Use PGD to compute solution to inner problem and find its generalized support $S$: Find $x$, s.t. 
    $$F(x,\lambda) - F(\hat{x},\lambda) < \epsilon.$$
    Compute the gradient $\nabla \mathcal{L}(\lambda)=\mathcal{J}_{S c }^{\top} \nabla_{S^c} \mathcal{C}(x)+\mathcal{J}_{S}^{\top} \nabla_{S} \mathcal{C}(x)$, with $\mathcal{J}_S$ as in Theorem  \ref{nonsmooth_gradient}.
    $$z = x - \gamma \nabla f(x).$$
    $$\mathcal{J}_{S^c:} = \partial_\lambda prox_{\gamma g(\cdot,\lambda)}(z)_{S^c}$$
    $$s = |S|$$
    $$A = \mathrm{Id}_s - \partial_z \mathrm{prox}_{\gamma g}(z)_S \odot (\mathrm{Id}_s - \gamma \nabla^2_{S,S} f(x))$$
    Solve the linear system up to tolerance $\epsilon$:
    $$A^\top v = \nabla_S \mathcal{C}(x).$$
    $$B = \left( \partial_\lambda prox_{\gamma g(\cdot,\lambda)}(\hat{z})_{\hat{S}} - \gamma \partial_z prox_{\gamma g(\cdot,\lambda)}(\hat{z})_{\hat{S}} \odot  \nabla^2_{\hat{S},\hat{S}^c} f(\hat{x}) \hat{\mathcal{J}}_{\hat{S}^c:}  \right)$$
    $$\nabla \mathcal{L}(\lambda) = \mathcal{J}_{S c }^{\top} \nabla_{S^c} \mathcal{C}(x) + v^T B.$$
    \Return $\mathcal{L}(\lambda) \triangleq \mathcal{C}(x)$,$\nabla \mathcal{L}(\lambda)$
  \caption{Active Set Identification + Outer Gradient Calculation} \label{alg:active_set}
 
\end{algorithm}

\begin{algorithm}[H]
 \KwData{Input $\lambda \in \mathbb{R}^r, \epsilon_k > 0$}
 %\KwResult{Updated weights $w^{[l]}_{jk}$ and biases $b^{[l]}_j$}

 \For{$k = 0,1,\hdots$}{
    $$\lambda_{old} = \lambda$$
    $$\mathcal{L}(\lambda),\nabla \mathcal{L}(\lambda) = \text{Algorithm \ref{alg:active_set}}(\lambda, \epsilon_k).$$
    $$\lambda -= \gamma_\lambda \nabla \mathcal{L}(\lambda)$$
 }
 \Return $\lambda$
 \caption{Gradient Descent with Approximate Gradient}
\end{algorithm}
Much like HOAG, this algorithm utilizes approximate gradients. Similar to Theorem \ref{thm_appgrad_hoag}, Bertrand et al. have shown that the error on the hypergradient $\nabla L(\lambda)$ is bounded by the order of the error in $x(\lambda)$ and the error in the resolution of the linear system. More specifically, we present their result below:
\begin{theorem}[Bound on the error of approximate gradient (Theorem 13 in \cite{bertrand2022implicit})]
For $\lambda \in \mathbb{R}^r$, let $\hat{x}(\lambda) \in \mathbb{R}^p$ be the exact solution of the inner problem (\ref{inprob_ns}), and $\hat{S}$ its generalized support. Suppose Assumptions \ref{smooth_a}, \ref{pcc_a} and \ref{locsmooth_a} hold. Let $\Lambda$ be a neighborhood of $\lambda$, and $\Gamma^{\Lambda} \triangleq$ $\left\{\hat{x}^{(\lambda)}-\gamma \nabla f\left(\hat{x}^{(\lambda)}\right): \lambda \in \Lambda\right\}$. Suppose hypotheses (H1) to (H4) from Theorem \ref{nonsmooth_gradient} are satisfied. In addition suppose
\begin{itemize}
    \item (H5) The map $x \mapsto \nabla^2 f(x)$ is Lipschitz continuous.
    \item (H6) The criterion $x \mapsto \nabla \mathcal{C}(x)$ is Lipschitz continuous.
    \item (H7) Both optimization problems in Algorithm \ref{alg:active_set} are solved up to precision $\epsilon$ with support identification: $\left\|x(\lambda)-\hat{x}^{(\lambda)}\right\| \leq \epsilon, A^{\top}$ is invertible, and $\left\|A^{-1 \top} \nabla_{\hat{S}} \mathcal{C}\left(x(\lambda)\right)-v\right\| \leq \epsilon$.
\end{itemize}
Then the error on the approximate hypergradient $h$ returned by Algorithm \ref{alg:active_set} is of the order of magnitude of the error $\epsilon$ on $x(\lambda)$ and $v$
$$
\|\nabla \mathcal{L}(\lambda)-h\|=\mathcal{O}(\epsilon).
$$
\end{theorem}
In short, if we solve the inner problem and the linear system in Algorithm \ref{alg:active_set} up to precision $\epsilon$, then the approximation on the hypergradient is also on the order of $\epsilon$. We omit the proof of this theorem, as it is similar to the proof of the boundedness of the hypergradient of HOAG and follows it closely.
...
%Note that Theorem 3\cite{poon} ensures that we always have a direction of descent, but only in the case of LASSO, as in \ref{inprob_vp_poon}, where. Generally speaking



%in the case of LASSO we can explicitly write (\ref{eq: fvphess}) as
%$$
%\nabla^2 f_{vp}(v)=\left(\begin{array}{cc}
%\operatorname{diag}\left(1-\left\|\xi_j\right\|_2^2\right)_{j \in J}+4 U^{\top} W U & 0 \\
%0 & \operatorname{diag}\left(1-\left\|\xi_j\right\|_2^2\right)_{j \in J^c}
%\end{array}\right),
%$$
%where $J$ denotes the support of $v$, $\xi = \frac{1}{\lambda}A^T(A(u \odot v)-b)$, $W = $ Furthermore, even though the problem is non-convex, by Theorem 3\cite{poon}, all of the saddle points of $f$ are either global minima or strict saddle points.

\subsection{Advantages of the smooth parametrization}
...
\end{equation}
with the regularizer $R$ having quadratic variational form as per (\ref{qvf_thm}), solving the smooth reparametrization (\ref{noncvx_varpro}) with LBFGS-B is often superior (or comparable) to the current state-of-the-art methods for directly tackling (\ref{reg_prob_poon}). In particular, this approach is less sensitive to the choice of $\lambda$. Furthermore, also by Theorem 2\cite{poon}, the gradient $\nabla f$ can be written in terms of the convex dual, with $L_y^*$ the convex dual of $L_y \triangleq L(\cdot,y)$, as
$$
...
\begin{equation*}
    p(\theta' \mid \mathcal{D}) = \sum_{\theta} P(\theta' | \theta)p(\theta \mid \mathcal{D}), \quad \forall \theta \in \mathbb{R}^d.
\end{equation*}
...
\begin{mdframed}[hidealllines=true,backgroundcolor=yellow!20]
    Bayesian Sampling section is too short. Will either merge this with Background on Stochastics or expand.
\end{mdframed}
...
Let $(\Omega, \mathcal{F}, (\mathcal{F}_t)_{t \geq 0}, \mathbb{P})$ be a filtered probability space and let $(B_t)_{t \geq 0}$ be a standard Brownian motion adapted to $(\mathcal{F}_t)$. Suppose $\theta = (\theta_t)_{t \geq 0}$ is an $(\mathcal{F}_t)$-adapted process such that
\[
\mathbb{E} \left[ \exp\left( \frac{1}{2} \int_0^T \theta_s^2 \, ds \right) \right] < \infty.
\]
Then the process
\[
M_t := \exp\left( \int_0^t \theta_s \, dB_s - \frac{1}{2} \int_0^t \theta_s^2 \, ds \right)
\]
...
\begin{proposition}[Novikov’s Condition]
Let $\theta = (\theta_t)_{t \geq 0}$ be an $(\mathcal{F}_t)$-adapted process. If
\[
\mathbb{E} \left[ \exp\left( \frac{1}{2} \int_0^T \theta_s^2 \, ds \right) \right] < \infty,
\]
...
Assume that $Z(X)$ defined by (5.2) is a martingale. Define a process $\tilde{W}=\left\{\tilde{W}_t=\left(\tilde{W}_t^{(1)}, \ldots\right.\right.$, $\left.\left.\tilde{W}_t^{(d)}\right), \tilde{\mathcal{F}}_t ; 0 \leq t<\infty\right\}$ by
...
\mathbb{E}\left[\int_0^t f(\ell) d B(\ell) \mid \mathcal{F}_s\right]=\int_0^s f(\ell) d B(\ell) \quad \forall t \geqslant s,
\end{equation}
...
\end{equation}
which describes the time evolution of expectations of functionals with respect to random variable $X_t$. Also of interest to us is the $L^2$ adjoint of the generator, also referred to as the Fokker-Planck operator and denoted $L^*$. Given test functions $f,g$, this operator satisfies
\begin{equation}
...
\begin{equation} \label{gen_dag_explicit}
    \mathcal{L}^* = -\mathrm{div}(b\cdot) + \frac{1}{2} \nabla^2:(\sigma \sigma^T)
\end{equation}
...
\end{equation}
is preserved over time. This reformulation allows us to investigate the spectral properties of $\mathcal{L}$, leading to functional inequalities that control the rate of convergence, such as the Poincar\'{e} and Log-Sobolev inequalities.
...
\end{equation*}
Now, denoting $\mathcal{L}^*$ as the adjoint of generator $\mathcal{L}$ in $L^2(\pi)$, we have, for $C^\infty$ and compactly supported functions $\phi, \varphi$,
    $$
\int_{\mathcal{X}}(\mathcal{L} \varphi) \phi \mathrm{d} \pi=\int_{\mathcal{X}} \varphi\left(\mathcal{L}^* \phi\right) \mathrm{d} \pi .
$$
From this, we can easily show that $\pi$ is an invariant probability measure of an SDE with generator $\mathcal{L}$ over $L^2(\pi)$ if and only if $\mathcal{L}^* 1 = 0$.
\end{comment}
...
\begin{equation}
a\beta |\nabla V(q)|^2 - \Delta V(q) \geq c, \quad \forall q \in \mathbb{R}^d \text{ such that } |q| \geq R,
\end{equation}
...
\begin{proof}
    ...
    \[
    \mathcal{H}(\psi(t) | \nu) \leq \mathcal{H}(\psi_0 | \nu) \exp(-2\beta^{-1}Rt), \quad \forall t \geq 0,
    \]
    and so
    \[\frac{\mathcal{H}(\psi(t) | \nu) - \mathcal{H}(\psi_0 | \nu)}{t} \leq \mathcal{H}(\psi_0 | \nu) \frac{\exp(-2\beta^{-1}Rt) - 1}{t}, \quad \forall t \geq 0.\]
...
\[
q \cdot \nabla V(q) \geq A |q|^2 - B, \quad \forall q \in \mathcal{D},
\]
it follows that:
\[
...
\end{aligned}
$$
hence the $(u,v)$-process (\ref{uvproc}) arises from the solution of the $(y,z,v)$-process (\ref{yzvproc}). By \cite[Theorem~3.1]{2025hadalangsamp}, given Assumption (\ref{reg1}), the process (\ref{uvproc}) also inherits properties from (\ref{yzvproc}) such as being well-posed, strong Feller, irreducible. The processes agree as long as the initial distributions agree, i.e., that the probability density function $\rho(t,\cdot)$ of $(u,v)_t$ and the probability density function $p(t,\cdot)$ of $(y,z,v)$ satisfy
...
$$
which is precisely our $v$ process with $G \equiv 0$. As such, in order to show well-posedness of our SDE, we consider \ref{uvproc} as perturbed CIR and OU processes. Using Girsanov' Theorem we will be able to translate the analysis of the latter via absorbing the data term $G$. In the following, 
...
\end{align*}
Using the fact that $x^\top \nabla G(x) \geq -\ccc$ by Assumption \ref{reg1} and taking expectation,
$$
...
$$
Provided that $ \alpha<\beta \lambda $, we have $\frac{d}{dt}\EE\bigl[y_t\bigr] \leq  \pa{\frac{3\alpha d}{\beta}  + 2\alpha \ccc} \EE\bigl[y_t\bigr]$ and therefore, by Gronwall's inequality,
$$
\EE\bigl[y_t\bigr]  \leq y_0 \exp\biggl( \alpha \pa{\frac{3 d}{\beta}  + 2 \ccc} t\biggr),
$$
where $y_0=\exp(\alpha \norm{z_0}^2/2)<\infty$. 
...
$
\EE\bigl[y_t\bigr]  \leq y_0 \exp\biggl(2\epsilon \gamma \cinfty^2  \pa{\frac{3 d}{\beta}  + 2 \ccc} t\biggr).
$
Returning to \eqref{eq:novikov}, for all $t\leq T-\epsilon$, we see
$$
\EE\left[ \exp\pa{\int_t^{t+\epsilon} \gamma \cinfty^2 \norm{z_s}^2} ds \right] \leq y_0\exp\biggl( 2\epsilon \gamma \cinfty^2  \pa{\frac{3 d}{\beta}  + 2 \ccc} T\biggr).
$$
Thus, Novikov's condition is satisfied. We can therefore apply Girsanov's theorem. The CIR process is positive almost surely as, following \cite{Dereich2011-tp}, each component has the form $d\hat u_t = ((4\kappa \lambda - \theta^2)/(8\hat u_t))\,dt - \frac12 \kappa \hat u_t\, dt + (\theta/2)\,d\hat W_t$ for $\theta^2 = 8/\beta$ and $2\kappa\lambda = \theta^2$, which satisfies the Feller positivity condition $2\kappa\lambda\geq \theta^2$.
By Girsanov, for all $T>0$, with the martingale defined by $M_T = \exp(-\int_0^T b(z_t) dt - \frac12 \int_0^T b(z_t)^2 dt)$, we see
$$
\EE\Bigl[1_{\{\exists t\in [0,T], \; u_t=0\}} M_T\Bigr] =\PP(\exists t\in [0,T], \;\hat u_t =0) =0.
$$
...
We are interested in the behaviour of dynamics (\ref{uvproc}) when $u$ is close to 0, in particular due to the $\frac{1}{u_t}$ term in the first equation. As such, we would like to derive moment bounds to investigate boundary behaviour of \ref{uvproc}. Firstly, \cite[Theorem~3.1]{2025hadalangsamp} provides moment bounds on $(u,v)$: 
\begin{equation} \label{uv_moment}
    \mean{\sup_{0\le t\le T} \norm{(u,v)_t}^q}<\infty,\text{ for any }T>0, \, q\ge 2
\end{equation}
These are obtained by expanding $d(\|u_t\|^2)^{q/2}$ and $d(\|v_t\|^2)^{q/2}$ via It\^{o}'s formula, using the fact that $x^\top \nabla G(x)$ is bounded below and using an approach such as in \cite[Theorem 4.4]{Mao2008-ur}. Let $L^q(\pi)$ denote the set of integrable functions $\phi\colon\mathcal X\to\real$ such that $\int \abs{\phi(x)}^q \pi(x)\,dx<\infty,$ where $\pi$ is the invariant measure \ref{pi}. In the following,  we show how to control expectations of $\phi((u,v)_t)$, which extends the moment bounds to $\phi\in L^q(\pi)$, and includes some $\phi$ that are singular near $u_i=0$.
...
2. For each $T>0$ and $\epsilon>0$, we have
 $\mean{\pp{\int_0^T \frac{1}{u_{i,s}^{2-\epsilon}}\,ds}^q} <\infty $ for all $q\ge 1$.
...
\begin{align*} |v_1 \,\nabla G(u\odot v) \,u_1^{\epsilon-1}|\le
...
\end{align*}
On the right-hand side, $q$th moments are finite, which we now demonstrate for the last two terms. By the Burkholder--Gundy--Davis inequality \cite[Theorem 3.28]{Karatzas1991-xr}, for a constant $c$,
\[
\mean{\abs{\int_0^T u_{1,t}^{\epsilon-1}\,dW^{1,1}_t}^q}
\le c\,
\pp{\int_0^T \mean{u_{1,t}^{2\epsilon-2}}\,dt}^{q/2}.
\]
The integrand on the right-hand side is bounded for $\epsilon>0$ by part 1 with $r=2-2\epsilon<2$. 
Under the assumption on $G$, we have that $v_1 [\nabla G (u\cdot v)]_1$ is polynomially growing and has bounded $q$th moments by \cite[Theorem~3.1]{2025hadalangsamp}.  Hence, $q$th moments of the integral term $c(\epsilon)\int \cdots\,dt$ are finite.
...
\begin{proposition} \label{timeregbnd} Let $(u,v)_t$ denote the solution of (\ref{uvproc}) with initial distribution $\rho_0$. Suppose that the assumptions of \ref{cor} hold. For $q\ge 1$, $T>0$, and $\epsilon\in(0,1/2)$,  there exists $c>0$ such that, for $0\le s,t\le T$,
\begin{enumerate}
...
\end{align*}
By Corollary \ref{cor}, the $\mathbb{E} \bp{\pp{\int_s^t\frac{1}{|u_r|^{2-\epsilon}} \,dr}^q}$ is finite for $\epsilon>0$ and $q\ge 1$. Hence, we have shown, for some constant $c$ depending on $\epsilon$ and $q$, that
...
In the last line, we exploit  $u_{i,s},u_{i,t}>\Delta$ outside $\mathcal R$.
The third factor is $O(|t-s|^{\epsilon/2})$ by part 1.
    Focus now on the first factor: for $(r,r')$ conjugate,
\begin{align*}
&\pp{\mathbb{E} \bp{\frac {|u_{i,t}-u_{i,s}|^{(1-\epsilon)\,p}}{|u_{i,s}\,u_{i,t}|^{(1-\epsilon)\,p}}}}
\le \pp{\mathbb{E} \bp{ {|u_{i,t}-u_{i,s}|^{(1-\epsilon)\,p\,r'}}}}^{1/r'}\, \pp{\mathbb{E} \bp{\frac {1}{|u_{i,s}\,u_{i,t}|^{(1-\epsilon)\,p \,r}}}}^{1/r}.
\end{align*}
...
We can achieve $(1-\epsilon)\,p\,r<1$ for any $\epsilon>0$, by choosing of both $p$ and $r$ close to $1$. The resulting estimate is
 \begin{align}\label{temporary}
  \mathbb{E} \bp{\frac {|u_{i,t}-u_{i,s}|}{u_{i,s}\,u_{i,t}} (1-1_{\mathcal R})}
 &\le c\, \abs{t-s}^{(1-\epsilon)/2}\, \Delta ^{-2\epsilon}\, \abs{t-s}^{\epsilon/2}
 = c\,|t-s|^{1/2}\, \Delta^{-2\epsilon}.
 \end{align}

    We have shown that 
       \begin{align}\label{combined_estimate}
       \mathbb{E} \bp{\frac {|u_{i,t}-u_{i,s}|}{u_{i,s}\,u_{i,t}}} 
     \le  c \,\Delta^{1-2\epsilon/(2+\epsilon)}
     +c\,|t-s|^{1/2} \,\Delta^{-2\epsilon}.
     \end{align}
     Taking $\Delta = \abs{t-s}^\alpha$ and equating powers of $\abs{t-s}$ in the right-hand-side
     leads to $\alpha\pa{1-2\epsilon/(2+\epsilon)} = \frac12 - 2\alpha \epsilon$. After relabeling in $\epsilon$, this implies an upper bound of $c \abs{t-s}^{\frac12 - \frac{\epsilon}{2}}$ provided that $\epsilon \leq \frac12$. 
  %   By letting $\Delta=\abs{t-s}^\alpha$ and equating powers of $\abs{t-s}$, we determine   $(1-\epsilon)\,\alpha=1/2-\alpha\epsilon$ and $\alpha=1/2$.
  % The outcome is an upper bound $c\,\abs{t-s}^{1/2-\epsilon}$. 
\end{proof}
...
\[\mathcal{H}(\pi_1\mid \pi_2)\le\frac{1}{2\,R} \mathcal{I}(\pi_1\mid \pi_2),\qquad \forall \pi_1\ll \pi_2. 
\]
...
\end{theorem}
%
\begin{proof} This follows from \cite[Theorem 2.25]{Lelievre2016-hx} if  a Lyapunov condition holds for $V$  and a minorisation conditions holds for the transition density.

The process $(u,v)_t$ is strong Feller and irreducible \cite[Theorem 3.1]{2025hadalangsamp}. Hence,  the minorisation conditions hold for any compact set~\cite[Lemma 2.3]{mattingly2002ergodicity}. 
...
\begin{lemma}[drift condition, \texorpdfstring{\cite[Lemma~4.1]{2025hadalangsamp}}{}]\label{lem:2ndmoments_discrete}
Let $V((u,v))=1+\|(u,v)\|^2$.
...
\begin{theorem}[Long-Term Convergence of Implicit-Explicit Scheme \texorpdfstring{\cite[Theorem~4.2]{2025hadalangsamp}}{}]
...
\begin{theorem}[\texorpdfstring{\cite[Theorem~4.3]{2025hadalangsamp}}{}]\label{thm:num_conv}
Under (\ref{reg3}), for any $T>0$,
$$
...
The proof of \ref{thm:num_conv}  is a consequence of \ref{prop:error_discrete} and \ref{prop:error_discrete_part2} below. We first make two simplifying assumptions, \ref{ass:2,ass:ball_R}, in order to derive convergence in \ref{prop:error_discrete}. We then show in \ref{prop:error_discrete_part2} that these two assumptions hold under our main assumption (\ref{reg3}). 
 %
...
% \end{itemize}
Let \ref{reg3} hold
with  $\nabla G$ being $L$-Lipschitz, $$\cinfty=\sup_{x} \norm{\nabla G(x)}_\infty<\infty,\qquad x^\top \nabla G(x) \ge -\ccc, \quad \forall x\in \mathcal X.$$ 
...
\begin{align*}
    ...
    \\& + \frac2\beta\norm{\deltaw_k}^2 + \sqrt{\frac8\beta}\Bigl((u^k)^\top \deltaw^1_k +  (v^k)^\top \deltaw^2_k\Bigr).
\end{align*}
Up to some  constant $C>0$, depending on $\cinfty$, $\lambda$, $d$, $\ccc$, and $\beta$,
\begin{align*}
     Z_{k+1}^2&\leq Z_k^2 + C\biggl(\deltat^2 Z_k^2 + \deltat + \norm{\deltaw_k}^2 + ((u,v)^k)^\top \deltaw_k\biggr)\\
     &\leq   Z_0^2 + C\biggl(\deltat^2 \sum_{j=0}^k Z_j^2 +T + \sum_{j=0}^k\norm{\deltaw_j}^2 + \sum_{j=0}^k ((u,v)^j)^\top \deltaw_j\biggr).
\end{align*}
...
\begin{align*}
  ...
  ^{\frac{p}2} 
  + (CT)^{\frac{p}2}  + C^{\frac{p}2}\Biggl(\sum_{j=0}^k\norm{\deltaw_j}^2\Biggl)^{\frac{p}2} + C^{\frac{p}2}\Biggl|\sum_{j=0}^k ((u,v)^j)^\top \deltaw_j\Biggr|^{\frac{p}2}\\
  &\leq Z_0^p + C^{\frac{p}2}  \,\deltat^p\, M^{\frac{p}{2}-1} \sum_{j=0}^M Z_j^{p} + (CT)^{\frac{p}{2}} + C^{\frac{p}{2}} M^{\frac{p}{2}-1} \sum_{j=1}^{M-1} \norm{\deltaw_j}^p + C^{\frac{p}{2}} \Biggl|\sum_{j=0}^{M-1} ((u,v)^j)^\top{\deltaw_j}\Biggr|^{p/2},
\end{align*}
where $M = \lfloor \frac{T}{\Delta t} \rfloor$. For the last two terms, observe that
  $\EE\sum_{j=0}^{M-1} \norm{\deltaw_j}^p \lesssim M (\deltat)^{p/2}$ and that, by the Burkholder--Davis--Gundy inequality, \begin{align*}
   \EE \Biggl|\sum_{j=0}^{M-1} ((u,v)^j))^\top {\deltaw_j}\Biggr|^{\frac{p}2} &\lesssim \EE \Biggl(\sum_{j=0}^{M-1} \norm{(u^j,v^j)}^2 \deltat\Biggr)^{\frac{p}4}
...
$
Under \ref{reg1}, the $p$th moment of the initial data $Z_0$ is finite and \ref{ass:2} holds for some $A<\infty$, which completes the proof.
\end{proof}

\subsection{Invariant Measure}
Finally, we want to show that the invariant measure of the discretization tends to the invariant measure of the continuous dynamics as $\Delta t \xrightarrow[]{} 0$. Using Proposition \ref{prop:error_discrete_part2} and Theorem \ref{thm_bndd_G}, we conclude that the stationary distribution of $(u,v)^k$ converges to $\pi$ in the following sense.
...
If $\pi_\tstep$ does not satisfy Assumption \ref{reg3}, we may replace $\pi_\tstep$ with  a regularized distribution $\tilde \pi_\tstep$. For example, let $\tilde{\pi}_\tstep(\cdot)=\pi_\tstep(\cdot \cap B_R) /\pi_\tstep(B_R)$ for the ball $B_R$ of radius $R=\tstep^{-1}$). An easy corollary of Lemma \ref{lem:2ndmoments_discrete} is that  $\pi_\tstep(V)$ (for $V((u,v))=1+\norm{(u,v)}^2$) is bounded uniformly in $\tstep$ ($\pi_\tstep(V) \leq \frac{R}{1-\alpha}$). Thus, $\pi_\tstep(B_R)\to 1$ and we have $\pi_\tstep(\phi)-\tilde\pi_\tstep(\phi)\to 0$ and $\tilde\pi_\tstep(\phi)$ uniformly bounded as $\tstep\to 0$ for any $1$-Lipschitz $\phi$. Then,  $E_{1,T}(\tstep)\to 0$  for
$
E_{1,T}(\tstep)\eqdef
\tilde\pi_{\tstep}(P_{T}(\phi))-\pi_\tstep(\phi)+(\pi_\tstep-\tilde\pi_\tstep)(\phi)$.

By geometric ergodicity of $(u,v)_t$ as described in \ref{erg2},
$P_T(\phi) - \pi(\phi)\le C\,\exp(-\rho\,T) \,V$
for some $\rho,C>0$. 
Take the average with respect to $\tilde\pi_\tstep$ to find
$\tilde\pi_\tstep(P_T(\phi)) - \pi(\phi)\le C\,\exp(-\rho\,T) \,\tilde\pi_\tstep(V)$.
We have that $\tilde\pi_\tstep$ is uniformly bounded, and  that $E_2(T)\to 0$ as $T\to \infty$ for
$E_2(T)\eqdef \tilde\pi_\tstep(P_T(\phi)) - \pi(\phi)$.
Adding the two contributions, we obtain
$
\pi_{\tstep}(\phi)- \pi(\phi)= E_{1,T}(\tstep)+E_2(T). $
For any $\epsilon>0$, we can choose $T$ large so the $\abs{E_2(T)}\le \epsilon/2$ and $\delta>0$  so  $\abs{E_{1,T}(\tstep)}\le\epsilon/2$ for $\tstep<\delta$. Then, we have 
$
|\pi_{\tstep}(\phi)- \pi(\phi)|=\epsilon$ if $\tstep<\delta$.
...
\subsection{1D Regression Problem}
...
Suppose Assumptions \ref{conv_ass}, \ref{smo_ass} and \ref{nablaFxl_bnd} hold. Choose $\eta$, such that $\eta \mu < 1.$ Denote the condition number as $\kappa = \frac{L}{\mu}$. Let Let $|\mathcal{B}_{Q-1+j}| = B(1-\eta\mu)^{j-1}$. Then the variance of the approximate hypergradient satisfies
...
An equivalent condition to $\mathcal{L}(\lambda)$ having $L_{\mathcal{L}}-$Lipschitz continuous gradient is that for any $\alpha,\beta \in \mathcal{D}:$
...
\begin{comment}
    \begin{equation} \label{varbndthm}
    \mathrm{Var}\left( \widehat{\nabla} \mathcal{L}(\lambda_k) \right) \leq 2\eta^2M^2\left(\frac{\kappa^2}{B_k (2 - \eta \mu)^2} + \frac{1}{|\mathcal{S}_{\nabla_{x} C,k}|(2\eta \mu - \eta^2 \mu^2)} \right)\left(\frac{L^2}{|\mathcal{S}_{\nabla_{x\lambda} F,k}|} + E^2\right)  +  \frac{\kappa^2M^2}{|\mathcal{S}_{\nabla_{x\lambda} F,k}|}+ \frac{M^2}{|\mathcal{S}_{\nabla_{\lambda} C,k}|} \triangleq D_{3,k}.
\end{equation}
\end{comment}
\begin{align}
\mathrm{Var}\left( \widehat{\nabla} \mathcal{L}(\lambda_k) \right) 
&\leq 2\eta^2 M^2 \left( 
    \frac{\kappa^2}{B_k (2 - \eta \mu)^2} 
    + \frac{1}{|\mathcal{S}_{\nabla_{x} C,k}| (2\eta \mu - \eta^2 \mu^2)} 
\right) \notag \\
&\quad \times \left( 
    \frac{L^2}{|\mathcal{S}_{\nabla_{x\lambda} F,k}|} + E^2 
\right) 
+ \frac{\kappa^2 M^2}{|\mathcal{S}_{\nabla_{x\lambda} F,k}|} 
...
\begin{comment}
Now, given our choice of $Q_k$ and $T_k$, using lemma \ref{sgdmu}, we have that
\[\mathbb{E}\|\mathbb{E}_{\lambda_k} \widehat{\nabla} \mathcal{L}(\lambda_k) - \nabla \mathcal{L}(\lambda_k)\|^2 \leq \mathbb{E}D_k = \mathcal{O}\left(\frac{1}{k^{1-s+\zeta}}\right).\]
Secondly, by our choice of $T$ and by lemma \ref{sgdmu}:
\begin{equation} \label{CSnukek}
    \sum_{k=1}^\infty \nu_k \mathbb{E}\left[\|\hat{x}(\lambda_k) - x_k\|^2 \right] \leq \sum_{k=1}^\infty \frac{1}{k^{1+\zeta}} < \infty;
\end{equation}
Similarly, by our choices in (\ref{fix_params_conv}), we have that
\begin{equation} \label{T2bound}
    \sum_{k=1}^\infty \nu_k^2 T_{2,k} \leq \sum_{k=1}^\infty \frac{1}{k^{1+\zeta}} < \infty;
\end{equation}    
\end{comment}
...
\section{Numerics}
\begin{mdframed}[hidealllines=true,backgroundcolor=yellow!20]
    If got time, add numerics comparing conditioning sensitivity of proposed algorithm to HOAG/ Stocbio.
\end{mdframed}
...
An equivalent condition to $L(\lambda)$ having Lipschitz continuous gradient is that for any $\alpha,\beta \in \mathcal{D}:$
\begin{equation} \label{lipschitz_equiv}
...
\begin{align}
    ...
    &= L(\lambda_k) - (\nabla L(\lambda_k) -p_k)\top(\lambda_k - \lambda_{k+1}) -\frac{1}{L}\|p_k\|^2+\frac{1}{2L}\|p_k\|^2 \quad \text{(using } \lambda_{k}-\lambda_{k+1} = \frac{1}{L}p_k) \\
...
\end{equation}
Now, by Theorem \ref{thm_appgrad_hoag}, $\|\nabla L(\lambda_k) -p_k\| = \mathcal{O}(\epsilon_k)$. As $\mathcal{D}$ is bounded (by Heine-Borel), we have that $\exists M>0$, such that
\[\|\nabla L(\lambda_k) -p_k\|\|\lambda_k - \lambda_{k+1}\| < M\epsilon_k.\]
...
We will now bound $\mathbb{E}M_i$ for $M_i = \left\| \prod_{j = Q-i+1}^Q A_j - A^{i} \right \|^2$. Note, that $M_0 = 0.$ As in the proof of proposition 3 in \cite{stocBio}, we write
