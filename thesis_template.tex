...
\part*{Introduction}
\begin{mdframed}[hidealllines=true,backgroundcolor=yellow!20]
    Have to think what exactly to put into first pages
\end{mdframed}
...
\chapter{Background on (Sparse) Optimization}
...
\begin{align*}
    ...
    &\argmin_{x \in \mathbb{R}^n} \frac{1}{2 \gamma} \|x - (x_k - \gamma f(x_k))\|_2^2 + g(x,\lambda) = \\
...
A nice feature of the proximal method is that it can also be formulated as a fixed-point iteration. Firstly, we state a useful result
\begin{lemma}[Prox and Subdifferential, \cite{proxalgs}] \label{prox_subdif}
Let $g$ be a proper, closed, convex function. Assume also that $g$ is subdifferentiable on its domain, that is, $\partial g$ is nonempty on the domain. The proximal operator $\mathrm{prox}_{\gamma g}$ and the subdifferential $\partial g$ are related as follows. For $\gamma > 0$:
...
Consider the problem
\[\min_x f(x) + g(x)\]
where $f: \mathbb{R}^n \xrightarrow[]{} \mathbb{R}$ and $g: \mathbb{R}^n \xrightarrow[]{} \mathbb{R} \cup \{+ \infty\}$ are closed proper convex and $f$ is differentiable. Let $\gamma > 0$. $x^*$ is a solution to the above problem if and only if it is a fixed point of
...
\begin{mdframed}[hidealllines=true,backgroundcolor=yellow!20]
    Talk about slow methods such as subgradient descent. Specialized methods, such as coordinate descent for $\ell_1$ and LARS. Expand on Primal-Dual. Splitting methods, such as Douglas-Rachford
\end{mdframed}
...
\chapter{Background and Existing methods}
...
\begin{mdframed}[hidealllines=true,backgroundcolor=yellow!20]
    Add overview of direct approaches for nonsmooth case, e.g., the algorithm of Bertrand and Klopfenstein, to differentiate the prox \cite{bertrand2022implicit}.
\end{mdframed}
