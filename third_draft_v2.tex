...
\section{Numerics}
We did not conduct an empirical study of the effect of conditioning on convergence, but we did
\begin{mdframed}[hidealllines=true,backgroundcolor=yellow!20]
...
\end{equation}
Now, by Theorem \ref{thm_appgrad_hoag}, $\|\nabla L(\lambda_k) -p_k\| = \mathcal{O}(\epsilon_k)$. As $\mathcal{D}$ is compact, it is bounded (by Heine-Borel). So there exists some radius $R$, such that $\|\lambda\| \leq R$ for all $\lambda \in \mathcal{D}$. Thus, we have that $\exists M>0$, such that
...
\]
Then, by submultiplicativity of norms:
\[
